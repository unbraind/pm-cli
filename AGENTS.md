...
- PM-command linked-test runs should default to `--pm-context schema` and only use `--pm-context tracker` when workspace tracker parity is explicitly required; rely on `run_results[].execution_context` metadata for parity diagnostics.
