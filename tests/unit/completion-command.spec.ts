...
describe("generateBashScript", () => {
  ...
  it("includes strict health flags in bash completion", () => {
    const script = generateBashScript();
    expect(script).toContain(
      "--strict-directories --strict-exit --fail-on-warn --check-only --check-telemetry --no-refresh --refresh-vectors --verbose-stale-items --json --quiet --path --no-extensions --no-pager --profile --help",
    );
  });
