...
describe("agent capability contracts", () => {
  it("projects stable capability groups and generated routing documentation", () => {
    ...
    expect(groups.find(({ family }) => family === "lifecycle")?.commands).toContain(
      "claim",
    );
    expect(groups.find(({ family }) => family === "extensions")?.commands).toContain(
      "package",
    );
  ...
  it("one tier-field edit changes CLI, MCP, completion, and docs projections", () => {
    const edited = PM_COMMAND_CAPABILITY_CONTRACTS.map((entry) =>
      entry.command === "context" ? { ...entry, tier: "internal" as const } : entry,
    );

    expect(listPmCommandsForTierFromContracts("core", edited)).not.toContain("context");
    expect(listPmCommandsForTierFromContracts("full", edited)).not.toContain("context");
    expect(
      listPmMcpToolsForProfileFromContracts(
  ...
  it("measures the compact core help against a one-screen budget", () => {
    const text = Array.from({ length: PM_CORE_HELP_BUDGET.max_lines }, () => "help")
      .join("\n");
    ...
    expect(measurePmCoreHelp("x".repeat(PM_CORE_HELP_BUDGET.max_utf8_bytes + 1))).toMatchObject({
      lines: 1,
      within_budget: false,
    });
  });
