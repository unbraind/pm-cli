...
describe("public merge-safety SDK primitives", () => {
  ...
  it("merges concurrent item collection appends and recomputes TOON headers", () => {
    ...
    theirs.metadata.notes = [
      { text: "theirs note", author: "agent-b", created_at: "2026-07-19T00:02:00.000Z" },
    ];
    ...
    expect(parsed.metadata.notes?.map((note) => note.text)).toEqual(["theirs note"]);
  ...
  it("preserves both divergent history suffixes and emits a valid chain", () => {
    ...
    expect(entries.map((entry) => entry.author)).toEqual(["seed", "agent-a", "agent-b"]);
  ...
  it("covers identical and one-sided history merges plus malformed inputs", () => {
    ...
    expect(mergeHistoryStreams(baseRaw, baseRaw, baseRaw).strategy).toBe("identical");
    expect(mergeHistoryStreams(baseRaw, advancedRaw, baseRaw).strategy).toBe("fast_forward_ours");
    expect(mergeHistoryStreams(baseRaw, baseRaw, advancedRaw).strategy).toBe("fast_forward_theirs");
    expect(() => mergeHistoryStreams("", "<<<<<<< ours\n", baseRaw)).toThrow(/conflict markers/);
    expect(() => mergeHistoryStreams("", "not-json\n", baseRaw)).toThrow(/invalid JSON/);
  });
  ...
  it("merges disjoint nested JSON keys and reports real leaf conflicts", () => {
    ...
    const conflict = mergeJsonDocuments('{"value":1}\n', '{"value":2}\n', '{"value":3}\n');
  ...
  it("covers item conflicts, add/add documents, and JSON add/delete semantics", () => {
    ...
    expect(parseItemDocument(merged.merged, { format: "toon" }).metadata.title).toBe("theirs");
    ...
    expect(() => mergeItemDocuments("", "invalid", serializeItemDocument(theirs, { format: "toon" }))).toThrow(
      /ours.*readable item document/,
    );

    expect(JSON.parse(mergeJsonDocuments("", "", '{"added":1}\n').merged)).toEqual({ added: 1 });
    expect(JSON.parse(mergeJsonDocuments('{"remove":1}\n', '{"remove":1}\n', "{}\n").merged)).toEqual({});
    expect(() => mergeJsonDocuments("{}", "<<<<<<< ours", "{}")).toThrow(/conflict markers/);
    expect(() => mergeJsonDocuments("{}", "invalid", "{}")).toThrow(/not valid JSON/);
  });

  it("covers deterministic tie ordering, collection deletions, and JSON delete conflicts", () => {
    const empty: ItemDocument = { metadata: {} as ItemDocument["metadata"], body: "" };
    const base = item("base", "2026-07-19T00:00:00.000Z");
    const create = createHistoryEntry({ nowIso: "2026-07-19T00:00:00.000Z", author: "seed", op: "create", before: empty, after: base });
    const oursUpdate = createHistoryEntry({ nowIso: "2026-07-19T00:01:00.000Z", author: "a", op: "update", before: base, after: item("a", "2026-07-19T00:01:00.000Z") });
    const oursSecond = createHistoryEntry({ nowIso: "2026-07-19T00:01:00.000Z", author: "z", op: "update", before: base, after: item("z", "2026-07-19T00:01:00.000Z") });
    const theirsUpdate = createHistoryEntry({ nowIso: "2026-07-19T00:01:00.000Z", author: "b", op: "update", before: base, after: item("b", "2026-07-19T00:01:00.000Z") });
    ...
    expect(parseItemDocument(collectionMerged.merged, { format: "toon" }).metadata.tags).toEqual([
      "keep",
      "ours",
      "theirs",
    ]);
    ...
    expect(mergeJsonDocuments('{"value":1}', '{"value":2}', '{}').conflict_paths).toEqual(["value"]);
    expect(mergeJsonDocuments('{"value":1}', '{"value":2}', '{}', { preferred: "theirs" }).merged).toBe("{}\n");
    expect(mergeJsonDocuments('{"value":1}', '{}', '{"value":2}').conflict_paths).toEqual(["value"]);
    expect(JSON.parse(mergeJsonDocuments('{"value":1}', '{}', '{"value":2}', { preferred: "theirs" }).merged)).toEqual({ value: 2 });
  ...
  it("executes the file-level driver protocol and leaves parseable output", async () => {
    ...
    await Promise.all([
      writeFile(base, serializeItemDocument(itemBase, { format: "toon" }), "utf8"),
      writeFile(ours, serializeItemDocument(item("ours", "2026-07-19T00:01:00.000Z"), { format: "toon" }), "utf8"),
      writeFile(theirs, serializeItemDocument(itemBase, { format: "toon" }), "utf8"),
    ]);
  ...
  it("runs item/history drivers and validates driver options", async () => {
    const workspace = await mkdtemp(path.join(os.tmpdir(), "pm-merge-driver-kinds-"));
    workspaces.push(workspace);
    ...
    ours.metadata.tags = ["ours"];
    const theirs = structuredClone(base);
    theirs.metadata.notes = [{ text: "theirs", created_at: "2026-07-19T00:01:00.000Z" }];
    await Promise.all([
      writeFile(basePath, serializeItemDocument(base, { format: "toon" }), "utf8"),
      writeFile(oursPath, serializeItemDocument(ours, { format: "toon" }), "utf8"),
      writeFile(theirsPath, serializeItemDocument(theirs, { format: "toon" }), "utf8"),
    ]);
    expect(
      (await runMergeDriver({ artifact: "item", basePath, oursPath, theirsPath, prefer: "theirs" }, {})).ok,
    ).toBe(true);

    const empty: ItemDocument = { metadata: {} as ItemDocument["metadata"], body: "" };
    const create = createHistoryEntry({ nowIso: "2026-07-19T00:00:00.000Z", author: "seed", op: "create", before: empty, after: base });
    const oursUpdate = createHistoryEntry({ nowIso: "2026-07-19T00:01:00.000Z", author: "a", op: "update", before: base, after: item("a", "2026-07-19T00:01:00.000Z") });
    const theirsUpdate = createHistoryEntry({ nowIso: "2026-07-19T00:02:00.000Z", author: "b", op: "update", before: base, after: item("b", "2026-07-19T00:02:00.000Z") });
    await Promise.all([
      writeFile(basePath, historyEntriesToRaw([create]), "utf8"),
      writeFile(oursPath, historyEntriesToRaw([create, oursUpdate]), "utf8"),
      writeFile(theirsPath, historyEntriesToRaw([create, theirsUpdate]), "utf8"),
    ]);
    const history = await runMergeDriver({ artifact: "history", basePath, oursPath, theirsPath }, {});
  ...
  it("installs idempotent git attributes/config and exercises the CLI adapter", async () => {
    await withTempPmPath(async (context) => {
      ...
      const wrongInstall = await context.runCliInProcess(["merge", "install", "extra", "--json"], {
        cwd: context.tempRoot,
        expectJson: true,
      });
      expect(wrongInstall.code).toBe(2);
      const missingDriver = await context.runCliInProcess(["merge", "driver", "json", "--json"], {
        cwd: context.tempRoot,
        expectJson: true,
      });
      expect(missingDriver.code).toBe(2);
      const unknown = await context.runCliInProcess(["merge", "unknown", "--json"], {
        cwd: context.tempRoot,
        expectJson: true,
      });
    ...
    await expect(runMergeInstall({}, { path: path.join(notGit, ".agents", "pm") })).rejects.toThrow(
      /not initialized/,
    );
  });
    ...
    await withTempPmPath(async (context) => {
      ...
      for (const extension of [".md", ""] as const) {
        ...
        await Promise.all([
          ...
          writeFile(theirsPath, serializeItemDocument(theirs, { format }), "utf8"),
        ]);
        const priorCwd = process.cwd();
        try {
          process.chdir(context.tempRoot);
          expect((await runMergeDriver({ artifact: "item", basePath, oursPath, theirsPath }, { path: context.pmPath })).ok).toBe(true);
        } finally {
          process.chdir(priorCwd);
        }
      }

      const nestedRepo = await mkdtemp(path.join(os.tmpdir(), "pm-merge-nested-git-"));
      workspaces.push(nestedRepo);
      execFileSync("git", ["init", "-q"], { cwd: nestedRepo });
      const priorCwd = process.cwd();
      try {
        process.chdir(nestedRepo);
        await expect(runMergeInstall({}, { path: context.pmPath })).rejects.toThrow(/outside the git repository/);
      } finally {
        process.chdir(priorCwd);
      }
    });
  ...
  it("rejects an initialized tracker outside a git repository", async () => {
    await withTempPmPath(async (context) => {
      ...
      try {
        process.chdir(context.tempRoot);
        await expect(runMergeInstall({}, { path: context.pmPath })).rejects.toThrow(/requires a git repository/);
      } finally {
        process.chdir(priorCwd);
      }
    });
  ...
  it("propagates non-missing gitattributes read failures", async () => {
    await withTempPmPath(async (context) => {
      ...
      try {
        process.chdir(context.tempRoot);
        await expect(runMergeInstall({}, { path: context.pmPath })).rejects.toMatchObject({
