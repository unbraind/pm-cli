...
import {
  _testOnly as commanderUsageTestOnly,
...
describe("CLI bootstrap and usage helper tails", () => {
  ...
  it("covers command usage ranking and package hint empty branches", async () => {
    ...
    const tempRoot = await mkdtemp(path.join(os.tmpdir(), "pm-commander-usage-"));
    const pmRoot = path.join(tempRoot, ".agents", "pm");
    const settingsPath = path.join(pmRoot, "settings.json");
    const typeRegistryModule = await import("../../../src/core/item/type-registry.js");
...
describe("CLI bootstrap entrypoints", () => {
  ...
  it("covers parseAsync numeric/help/generic catch branches in runPmCli", async () => {
    await withTempPmPath(async (context) => {
      const parseSpy = vi.spyOn(Command.prototype, "parseAsync");
      ...
      try {
        ...
        expect(generic.code).toBe(EXIT_CODE.GENERIC_FAILURE);
        expect(generic.stderr).toContain("parse exploded");
      } finally {
        bootstrapHelpSpy.mockRestore();
  ...
  it("surfaces hook and flush reporting failures in generic error handling", async () => {
    ...
    const clearSpy = vi.spyOn(extensions, "clearActiveExtensionHooks").mockImplementationOnce(() => {
      throw new Error("clear hooks failed");
    });
    const flushSpy = vi.spyOn(sentryHelpers, "sentryFlush").mockRejectedValueOnce(new Error("flush failed"));
    try {
      ...
      expect(stderr).toContain("Failed to run error hooks: clear hooks failed");
      expect(stderr).toContain("Failed to flush error reporting: flush failed");
    } finally {
...
`,
  ...
  it("returns null from maybeLoad when discovery snapshot is unavailable", async () => {
    const tempRoot = await mkdtemp(path.join(os.tmpdir(), "pm-runtime-maybe-load-file-root-"));
    const fileRoot = path.join(tempRoot, "not-a-directory");
    try {
      await writeFile(fileRoot, "not a tracker directory", "utf8");
      const root = new Command().name("pm");
  ...
  it("returns an empty runtime snapshot when activation handlers fail after discovery", async () => {
    await withTempPmPath(async (context) => {
      await writeTestExtension({
        root: path.join(context.pmPath, "extensions"),
        directory: "late-good",
...
`,
      ...
      const late = root.command("late");
      late.setOptionValue("pmPath", context.pmPath);
      await expect(_testOnly.maybeLoadRuntimeExtensions(late)).resolves.toMatchObject({
        commands: { handlers: [] },
...
`,
  ...
  it("skips blank and descriptorless cached dynamic command paths safely", async () => {
    await withTempPmPath(async (context) => {
      await writeTestExtension({
        root: path.join(context.pmPath, "extensions"),
        directory: "dynamic-cache-mutator",
        manifest: {
          name: "dynamic-cache-mutator",
          capabilities: ["commands"],
          entry: "./index.mjs",
        },
        entryFilename: "index.mjs",
...
export default {
  activate(api) {
    api.registerCommand({ name: "tools export", run: () => ({ ok: true }) });
  }
};
`,
      });
      ...
      snapshot.commandHandlers.push("   ");
      snapshot.commandHandlers.push("ghost");
      snapshot.commandDescriptors.delete("ghost");
...
describe("CLI Commander usage recovery helpers", () => {
  ...
  it("resolves child commands and partial help paths through aliases", () => {
    ...
    const nonFunctionAliasesParent = {
      commands: [
        {
          name: () => "shadow",
          aliases: ["sh"],
  ...
  it("covers package-hint and flag-suggestion helper edge branches", () => {
    expect(commanderUsageTestOnly.resolveOptionalPackageInstallHint("   ")).toBeNull();
  ...
  it("falls back to a default commander usage message for non-object errors", async () => {
    const previousArgv = process.argv;
    try {
      process.argv = ["node", "pm", "list"];
      const context = await resolveCommanderUsageContext("plain error", new Command().name("pm"), new Map());
      expect(context.message).toBe("Invalid command usage");
    } finally {
      process.argv = previousArgv;
    }
  });
...
describe("CLI rich help content", () => {
  it("covers JSON help option helper fallbacks and alias compaction sorting", () => {
    expect(helpJsonTestOnly.readOptionAttributeName({})).toBeNull();
    expect(helpJsonTestOnly.buildHelpOptionSummaries({} as never)).toEqual([]);
    const aliasMap = helpJsonTestOnly.buildOptionAliasMap([
      { long: "--title", attributeName: () => "title" },
      { long: "--title-alt", attributeName: "title" },
      { long: "   ", attributeName: "title" },
      { long: "--missing-attr" },
    ]);
    expect(aliasMap.get("title")).toEqual(["--title", "--title-alt"]);

    const compacted = helpJsonTestOnly.compactHelpOptionAliases([
      {
        flags: "--title <value>",
        long: "--title",
        short: null,
    ...
    expect(compacted).toEqual([
      expect.objectContaining({
        long: "--title",
        aliases: ["--title-a", "--title-z"],
      }),
    ]);
    expect(
      helpJsonTestOnly.compactHelpOptionAliases([
        {
          flags: "--count <value>",
          long: "--count",
          short: null,
  ...
  it("covers unknown-help runtime fallback payload and quiet/noisy render branches", async () => {
    const program = new Command().name("pm").option("--json", "JSON output").option("--quiet", "Quiet");
