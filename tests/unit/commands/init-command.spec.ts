...
import { EXIT_CODE, PM_REQUIRED_SUBDIRS } from "../../../src/core/shared/constants.js";
...
describe("runInit", () => {
  ...
  it("covers agent guidance helper branches without prompting", () => {
    ...
    expect(initGuidanceInternals.ensureTrailingNewline("missing")).toBe("missing\n");
    ...
    expect(initGuidanceInternals.findGuidanceBlockRange("<!-- pm-cli:agent-guidance:start:v1 -->\nmissing end")).toBeNull();
    ...
    expect(initGuidanceInternals.normalizeAgentGuidanceState({} as Parameters<typeof initGuidanceInternals.normalizeAgentGuidanceState>[0])).toEqual({
    ...
    const nextSteps = ["Add workflow guidance later: pm init --agent-guidance add"];
    initGuidanceInternals.pushUnique(nextSteps, "Add workflow guidance later: pm init --agent-guidance add");
    expect(nextSteps).toEqual(["Add workflow guidance later: pm init --agent-guidance add"]);
  });

  it("covers stale-scan no-op add and ask guidance write branches", async () => {
    const projectRoot = await mkdtemp(path.join(os.tmpdir(), "pm-init-guidance-stale-scan-"));
    ...
    try {
      {
        ...
        const pathExistsMock = vi.fn(async (filePath: string) => filePath === guidancePath);
        ...
        try {
          const mockedModule = await import("../../../src/cli/commands/init-agent-guidance.js");
          const addResult = await mockedModule.runInitAgentGuidance({
            pm_root: pmRoot,
            cwd: projectRoot,
            mode: "add",
            interactive: false,
            settings: {} as Parameters<typeof mockedModule.runInitAgentGuidance>[0]["settings"],
          });
          expect(addResult.summary.applied).toBe(false);
          expect(addResult.warnings.some((warning) => warning.startsWith("agent_guidance:added:"))).toBe(false);
...

      {
        ...
        try {
          ...
          expect(askResult.warnings.some((warning) => warning.startsWith("agent_guidance:added:"))).toBe(false);
          expect(writeFileMock).not.toHaveBeenCalled();
        } finally {
  ...
  it("preserves prior declined_at on explicit skip and keeps present guidance in ask mode", async () => {
    const tempRoot = await mkdtemp(path.join(os.tmpdir(), "pm-init-guidance-branch-state-"));
    try {
      await writeFile(tempRoot + "/AGENTS.md", initGuidanceInternals.buildAgentGuidanceBlock("\n"), "utf8");
  ...
  it("fails init --with-packages when bundled installs report unsuccessful entries", async () => {
    ...
    vi.doMock("../../../src/cli/commands/extension.js", async () => {
      const actual = await vi.importActual<typeof import("../../../src/cli/commands/extension.js")>(
    ...
    try {
      ...
      const tempRoot = await mkdtemp(path.join(os.tmpdir(), "pm-init-with-packages-fail-"));
      try {
        await expect(
  ...
  it("uses the default readline factory when no test factory is configured", async () => {
    ...
    try {
      const mockedInitModule = await import("../../../src/cli/commands/init.js");
      const firstChoices = await mockedInitModule._testOnly.runInitWizard("pm-", true);
      ...
      const secondChoices = await mockedInitModule._testOnly.runInitWizard("pm-", true);
  ...
  it("uses default agent-guidance readline fallback and no-op write path", async () => {
    ...
    const writeSpy = vi.spyOn(process.stdout, "write").mockImplementation(() => true);
    const tempRoot = await mkdtemp(path.join(os.tmpdir(), "pm-init-guidance-default-factory-"));

    try {
      const mockedGuidanceModule = await import("../../../src/cli/commands/init-agent-guidance.js");
      const declinedPrompt = await mockedGuidanceModule._testOnly.promptForGuidanceWrite("AGENTS.md");
      expect(declinedPrompt).toBe(false);
      mockedGuidanceModule._testOnly.setAgentGuidanceReadlineFactoryForTests(undefined);
      const acceptedPrompt = await mockedGuidanceModule._testOnly.promptForGuidanceWrite("AGENTS.md");
      expect(acceptedPrompt).toBe(true);
      expect(createInterfaceMock).toHaveBeenCalledTimes(2);
      expect(close).toHaveBeenCalledTimes(2);

      const guidancePath = path.join(tempRoot, "AGENTS.md");
      const normalizedGuidance = mockedGuidanceModule._testOnly.upsertAgentGuidanceBlock("# Existing header").next_content;
      await writeFile(guidancePath, normalizedGuidance, "utf8");
      const noChangeWrite = await mockedGuidanceModule._testOnly.writeGuidanceFile(guidancePath);
  ...
  it("deduplicates guidance next steps that already exist in init output", async () => {
    ...
    const runInitAgentGuidanceMock = vi.fn(async () => ({
      ...
      warnings: [],
      next_steps: ["Set PM_AUTHOR=<your-agent-id> so mutations attribute to the right caller."],
      settings_changed: false,
    }));

    vi.doMock("../../../src/cli/commands/init-agent-guidance.js", async () => {
      const actual = await vi.importActual<typeof import("../../../src/cli/commands/init-agent-guidance.js")>(
    ...
    try {
      const initModule = await import("../../../src/cli/commands/init.js");
      const tempRoot = await mkdtemp(path.join(os.tmpdir(), "pm-init-next-step-dedupe-"));
      try {
        const result = await initModule.runInit("pm", { path: tempRoot }, { defaults: true, agentGuidance: "status" });
        expect(
          result.next_steps.filter(
            (step) => step === "Set PM_AUTHOR=<your-agent-id> so mutations attribute to the right caller.",
          ),
        ).toHaveLength(1);
      } finally {
        await rm(tempRoot, { recursive: true, force: true });
  ...
  it("keeps agent guidance unchanged when add mode runs on an already-upserted file", async () => {
    const tempRoot = await mkdtemp(path.join(os.tmpdir(), "pm-init-guidance-noop-"));
    try {
      await runInit("pm", { path: tempRoot }, { defaults: true, agentGuidance: "status" });
      const settings = await readSettings(tempRoot);
