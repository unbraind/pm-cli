...
describe("calendar command module", () => {
  it("resolves output format precedence and conflicts", () => {
    ...
    expect(() => resolveCalendarOutputFormat({ format: "jsoon" }, { json: false })).toThrow(
      'Did you mean "json"?',
    );
  });
  ...
  it("ignores invalid exdates and handles zero-duration recurring event windows", async () => {
    await withTempPmPath(async (context) => {
      const exdateItem = createCalendarItem(context, {
        title: "Invalid exdate seed",
        tags: "calendar,invalid-exdate",
        events: ["start=2026-04-01T09:00:00.000Z,title=Invalid exdate recurring,recur_freq=daily,recur_count=3"],
      });
      await rewriteFirstEventExdates(context, exdateItem, [
        "not-a-timestamp",
        "2026-04-02T09:00:00.000Z",
      ]);

      createCalendarItem(context, {
        title: "Zero-duration recurring seed",
        tags: "calendar,invalid-exdate",
        events: ["start=2026-04-01T10:00:00.000Z,end=2026-04-01T10:00:00.000Z,title=Zero duration,recur_freq=daily,recur_count=2"],
      });
      ...
      expect(result.events.map((event) => event.at)).toContain("2026-04-03T09:00:00.000Z");
      expect(result.events.some((event) => event.item_title === "Zero-duration recurring seed")).toBe(true);
    });
  });
  ...
  it("suggests close view names and validates assignee filter conflicts", async () => {
    await withTempPmPath(async (context) => {
      createCalendarItem(context, {
        title: "Assignee validation seed",
        assignee: "agent-a",
        deadline: "2026-04-10T10:00:00.000Z",
      });
      ...
      await expect(
        ...
        exitCode: EXIT_CODE.USAGE,
        message: expect.stringContaining('Did you mean "agenda"?'),
      });
      ...
      await expect(
        ...
        exitCode: EXIT_CODE.USAGE,
        message: "Calendar assignee filter must be one of assigned|unassigned",
      });
      ...
      await expect(
        ...
        exitCode: EXIT_CODE.USAGE,
        message: 'Invalid calendar assignee filter "unknown". Allowed: assigned|unassigned',
      });
      ...
      await expect(
        ...
        exitCode: EXIT_CODE.USAGE,
        message: '--assignee no longer accepts "none" or "null". Use --assignee-filter unassigned.',
      });
  ...
  it("includes recurrence interval details in event summaries", async () => {
    await withTempPmPath(async (context) => {
      createCalendarItem(context, {
        title: "Interval recurrence seed",
        tags: "calendar,interval-summary",
        events: ["start=2026-04-01T09:00:00.000Z,title=Interval event,recur_freq=daily,recur_interval=2,recur_count=3"],
      });
      ...
      const markdown = renderCalendarMarkdown(result);
      expect(markdown).toContain("interval=2");
    });
  });

  it("filters assigned mode and runtime metadata together", async () => {
    await withTempPmPath(async (context) => {
      createCalendarItem(context, {
        title: "Assigned runtime-mismatch seed",
        assignee: "agent-a",
        deadline: "2026-04-11T10:00:00.000Z",
      });
      createCalendarItem(context, {
        title: "Unassigned runtime-mismatch seed",
        deadline: "2026-04-11T11:00:00.000Z",
      });
      ...
      settings.schema.fields = [
        ...(settings.schema.fields ?? []),
        {
          key: "calendar_runtime_field",
          type: "string",
          commands: ["calendar"],
        },
      ];
      await writeSettings(context.pmPath, settings, "calendar:runtime-field");
      ...
      const filtered = await runCalendar(
        {
          ...
          calendarRuntimeField: "enterprise",
        } as unknown as CalendarOptions,
        { path: context.pmPath },
      );
      expect(filtered.summary.events).toBe(0);
      expect(filtered.events).toHaveLength(0);
    });
  });

  it("renders reminder/event fallback text and active-window period mode", () => {
    const reminderEvent = {
      kind: "reminder",
      at: "2026-04-12T09:00:00.000Z",
      date: "2026-04-12",
      item_id: "pm-1",
      item_title: "Reminder fallback title",
      item_type: "Task",
      item_status: "open",
      item_priority: 1,
      item_deadline: null,
    ...
    const scheduledEvent = {
      ...
      date: "2026-04-12",
      item_id: "pm-1",
      item_title: "Event fallback title",
      item_type: "Task",
      item_status: "open",
      item_priority: 1,
      item_deadline: null,
    ...
    const markdown = renderCalendarMarkdown({
      ...
      anchor: "2026-04-12T00:00:00.000Z",
      range: {
        start: "2026-04-12T00:00:00.000Z",
        end: "2026-04-13T00:00:00.000Z",
        period_start: "2026-04-12T00:00:00.000Z",
      ...
      summary: {
        ...
        scheduled: 1,
        items: 1,
        by_kind: { event: 1, reminder: 1 },
        by_type: { Task: 2 },
        by_status: { open: 2 },
        recurring_events: 0,
      },
      events: [reminderEvent, scheduledEvent],
      days: [
        {
          date: "2026-04-12",
          count: 2,
      ...
      warnings: [],
    } as Parameters<typeof renderCalendarMarkdown>[0]);
    expect(markdown).toContain("period-mode: active-window");
    expect(markdown).toContain("reminder=\"\"");
    expect(markdown).toContain("[event] pm-1");
  });

  it("sorts summary records and warning lists deterministically", async () => {
    await withTempPmPath(async (context) => {
      createCalendarItem(context, {
        title: "Summary sort task",
        type: "Task",
        tags: "calendar,summary-sort",
        deadline: "2026-04-13T12:00:00.000Z",
        reminders: ["at=2026-04-13T09:00:00.000Z,text=summary reminder"],
        events: ["start=2026-04-13T14:00:00.000Z,title=summary event"],
      });
      createCalendarItem(context, {
        title: "Summary sort issue",
        type: "Issue",
        tags: "calendar,summary-sort",
        deadline: "2026-04-13T16:00:00.000Z",
      });
      await writeFile(path.join(context.pmPath, "tasks", "pm-invalid-calendar.toon"), "{not-valid-toon", "utf8");

      const summaryResult = await runCalendar(
        {
          view: "agenda",
      ...
      expect(summaryResult.summary.by_kind.deadline).toBe(2);
      expect(summaryResult.summary.by_kind.reminder).toBe(1);
      expect(summaryResult.summary.by_kind.event).toBe(1);
      expect(Object.keys(summaryResult.summary.by_type)).toEqual(["Issue", "Task"]);

      const warningResult = await runCalendar(
        {
          view: "agenda",
          include: "events",
          tag: "summary-sort",
      ...
      expect((warningResult.warnings ?? []).length).toBeGreaterThan(1);
      expect(warningResult.warnings).toEqual([...(warningResult.warnings ?? [])].sort((left, right) => left.localeCompare(right)));
    });
  });
    ...
    await withTempPmPath(async (context) => {
      ...
      const listAllItemMetadataLightMock = vi.fn(
        async (
          _pmRoot: string,
          _itemFormat: string,
          _typeToFolder: Record<string, string>,
          warnings: string[],
        ) => {
          warnings.push("warning-zeta");
          warnings.push("warning-alpha");
          return [
            {
              id: "pm-mocked-calendar",
              title: "Mocked calendar item",
              type: "Task",
              status: "open",
              priority: 1,
              tags: ["calendar", "mocked"],
              assignee: null,
              deadline: null,
              reminders: [],
              events: [
                {
                  ...
                  recurrence: {
                    ...
                    interval: 1,
                    count: 2,
                    exdates: ["not-a-date", "2026-04-02T10:00:00.000Z"],
                  },
                },
                {
                  start_at: "2026-04-01T12:00:00.000Z",
                  end_at: "2026-04-01T13:00:00.000Z",
                  title: "Duration carry-forward",
                  description: null,
                  all_day: false,
                  timezone: null,
                  location: null,
