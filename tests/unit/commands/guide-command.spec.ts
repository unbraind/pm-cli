...
describe("guide command runtime", () => {
  ...
  it("throws usage error for unknown topic", async () => {
    ...
    expect(() => _testOnlyGuide.ensureGuideTopic("")).toThrow('Unknown guide topic ""');
  });
  ...
  it("resolves explicit output formats and rejects unknown formats", () => {
    ...
    expect(resolveGuideOutputFormat({}, GLOBAL_OPTIONS)).toBe("toon");
  ...
  it("renders optional-missing doc states and omits related section when empty", () => {
    const markdown = renderGuideMarkdown({
      output_default: "toon",
      mode: "topic",
      depth: "brief",
      requested_topic: "synthetic",
      topic: {
        id: "synthetic",
        aliases: [],
        title: "Synthetic",
    ...
    expect(markdown).toContain("missing (optional)");
    expect(markdown).not.toContain("## Related topics");
  });
});
...
describe("guide doc resolution", () => {
  ...
  it("truncates standard excerpts by max character budget", async () => {
    tempRoot = await mkdtemp(path.join(os.tmpdir(), "pm-guide-char-truncate-"));
    await mkdir(path.join(tempRoot, "docs"), { recursive: true });
    await writeFile(path.join(tempRoot, "README.md"), "# temp\n", "utf8");
    await writeFile(path.join(tempRoot, "docs", "QUICKSTART.md"), `${"x".repeat(12_500)}\n`, "utf8");
    ...
    const result = await runGuide({ topic: "quickstart", depth: "standard" }, GLOBAL_OPTIONS);
    const quickstartDoc = result.docs.find((doc) => doc.path === "docs/QUICKSTART.md");
    expect(quickstartDoc).toMatchObject({
      exists: true,
      content_mode: "excerpt",
      truncated: true,
      line_count: 2,
    });
    expect((quickstartDoc?.content ?? "").length).toBe(12_000);
  });
