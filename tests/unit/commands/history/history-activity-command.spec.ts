...
import {
  _testOnly as activityInternals,
  runActivity,
} from "../../../../src/cli/commands/history/activity.js";
import {
  readHistoryEntries,
  runHistory,
} from "../../../../src/cli/commands/history/history.js";
import {
  _testOnly as historyRedactInternals,
  runHistoryRedact,
} from "../../../../src/cli/commands/history/history-redact.js";
import {
  _testOnlyRestoreCommand,
  runRestore,
} from "../../../../src/cli/commands/history/restore.js";
import {
...
describe("runHistory and runActivity", () => {
  ...
  it("covers history-redact pure helper branches", () => {
    expect(historyRedactInternals.normalizeStringArrayInput(" one ")).toEqual([
      " one ",
    ]);
    expect(
      historyRedactInternals.normalizeStringArrayInput(["a", "b"]),
    ).toEqual(["a", "b"]);
    ...
    const rules = historyRedactInternals.buildRedactionRules(
      [" secret ", "secret"],
      "/token-[0-9]+/",
    );
    ...
    expect(historyRedactInternals.applyLiteralRule("aaaa", "aa", "b")).toEqual({
      value: "bb",
      replacements: 2,
    });
    expect(historyRedactInternals.applyLiteralRule("aaaa", "", "b")).toEqual({
      value: "aaaa",
      replacements: 0,
    });
    expect(
      historyRedactInternals.applyRegexRule(
        "token-1 token-x",
        historyRedactInternals.parseRegexRule("/token-[0-9]+/"),
        "x",
      ),
    ).toEqual({
      value: "x token-x",
    ...
    expect(
      historyRedactInternals.redactStringValue("secret token-1", rules, "x"),
    ).toEqual({ value: "x x", replacements: 2 });
    expect(
      historyRedactInternals.redactUnknownValue(
        { nested: ["secret", 3, { token: "token-2" }] },
        rules,
        "x",
      ),
    ).toEqual({
    ...
    expect(
      historyRedactInternals.hasItemMetadata({
        metadata: { id: "pm-a" },
        body: "",
        events: [],
      } as never),
  ...
  it("covers restore target and replay helper branches", () => {
    ...
    expect(
      _testOnlyRestoreCommand.ensureReplayTarget("1", history),
    ).toMatchObject({ kind: "version", historyIndex: 0 });
    expect(
      _testOnlyRestoreCommand.ensureReplayTarget(
    ...
    expect(() =>
      _testOnlyRestoreCommand.ensureReplayTarget(" ", history),
    ).toThrow("Missing restore target");
    expect(() =>
      _testOnlyRestoreCommand.ensureReplayTarget("3", history),
    ).toThrow("Restore version must");
    expect(() =>
      _testOnlyRestoreCommand.ensureReplayTarget("not-a-date", history),
    ).toThrow("Invalid restore target");
    expect(() =>
      _testOnlyRestoreCommand.ensureReplayTarget(
        "2025-12-31T00:00:00.000Z",
        history,
      ),
    ).toThrow("No history entries exist at or before");

    expect(
      _testOnlyRestoreCommand.replayToTarget(history, 1).metadata,
    ).toMatchObject({ title: "A2", priority: 2 });
    expect(
      _testOnlyRestoreCommand.replayCurrentDocument(history).metadata,
    ).toMatchObject({ title: "A2" });
    expect(
    ...
    expect(
      _testOnlyRestoreCommand.extractPatchFailureContext(
        ...
        {
          index: 0,
          operation: { op: "replace", path: "/metadata/title", from: "/old" },
        },
      ),
    ).toEqual({
      patchIndex: 0,
    ...
    expect(() =>
      _testOnlyRestoreCommand.ensureMaterializedRestoreTarget(
        EMPTY_REPLAY_DOCUMENT,
        {
          kind: "version",
          raw: "1",
  ...
  it("restores an item to an earlier history version", async () => {
    await withTempPmPath(async (context) => {
      ...
      const result = await runRestore(
        id,
        "1",
        { author: "test-author", message: "restore initial" },
        { path: context.pmPath },
      );

      expect(result.restored_from).toMatchObject({
        kind: "version",
        target: "1",
        history_index: 1,
        entry_op: "create",
      ...
      const restored = context.runCli(["get", id, "--json"], {
        expectJson: true,
      });
      expect(
        (restored.json as { item: { priority: number } }).item.priority,
      ).toBe(1);
    });
  });
  ...
  it("fails when tracker is not initialized", async () => {
    const tempDir = await mkdtemp(
      path.join(os.tmpdir(), "pm-history-not-init-"),
    );
    try {
      await expect(
        runHistory("pm-missing", {}, { path: tempDir }),
      ).rejects.toMatchObject<PmCliError>({
        exitCode: EXIT_CODE.NOT_FOUND,
      });
      await expect(
        runActivity({}, { path: tempDir }),
      ).rejects.toMatchObject<PmCliError>({
        exitCode: EXIT_CODE.NOT_FOUND,
      });
    } finally {
  ...
  it("validates --limit values", async () => {
    await withTempPmPath(async (context) => {
      ...
      await expect(
        runHistory(id, { limit: "-1" }, { path: context.pmPath }),
      ).rejects.toMatchObject<PmCliError>({
        exitCode: EXIT_CODE.USAGE,
      });
      await expect(
        runActivity({ limit: "not-a-number" }, { path: context.pmPath }),
      ).rejects.toMatchObject<PmCliError>({
        exitCode: EXIT_CODE.USAGE,
      });
      await expect(
  ...
  it("returns item history with optional limiting and handles empty/missing history files", async () => {
    await withTempPmPath(async (context) => {
      ...
      const limited = await runHistory(
        id,
        { limit: "1" },
        { path: context.pmPath },
      );
      ...
      const cliDefault = context.runCli(["history", id, "--json"], {
        expectJson: true,
      });
      ...
      const compactDiff = context.runCli(["history", id, "--json", "--diff"], {
        expectJson: true,
      });
      ...
      expect(
        compactDiffEntries?.some((entry) =>
          entry.changed_fields.includes("body"),
        ),
      ).toBe(true);

      const cliFull = context.runCli(["history", id, "--json", "--full"], {
        expectJson: true,
      });
      expect(cliFull.code).toBe(0);
      expect(cliFull.json).toMatchObject({ compact: false });
      expect(
        (cliFull.json as { history?: unknown[] }).history?.length,
      ).toBeGreaterThan(0);

      const conflictingProjection = context.runCli([
        "history",
        id,
        "--compact",
        "--full",
      ...
      expect(conflictingProjection.stderr).toContain(
        "History projection options are mutually exclusive",
      );
  ...
  it("supports additive diff output for history entries", async () => {
    await withTempPmPath(async (context) => {
      ...
      expect(
        all.diff?.some((entry) => entry.changed_fields.includes("body")),
      ).toBe(true);
  ...
  it("renders compact history when legacy entries have missing patch arrays", async () => {
    await withTempPmPath(async (context) => {
      ...
      const legacyEntry = compact.compact_history?.find(
        (entry) => entry.op === "legacy-import",
      );
  ...
  it("supports additive verification output for history hash chains", async () => {
    await withTempPmPath(async (context) => {
      ...
      const tampered = await runHistory(
        id,
        { verify: true },
        { path: context.pmPath },
      );
  ...
  it("verifies orphaned history streams when no item file exists", async () => {
    await withTempPmPath(async (context) => {
      ...
      const orphanHistoryPath = path.join(
        context.pmPath,
        "history",
        `${orphanId}.jsonl`,
      );
  ...
  it("returns structured verification errors when history replay cannot apply a patch", async () => {
    await withTempPmPath(async (context) => {
      ...
      expect(tampered.verification?.ok).toBe(false);
      expect(tampered.verification?.errors).toEqual([
    ...
    await withTempPmPath(async (context) => {
      ...
      context.runCli(
        [
          ...
          "--json",
          "--body",
          "ticket secret-123 and secret-456",
          "--author",
          "test-author",
          "--message",
          "append regex tokens",
  ...
  it("moves item files when redaction changes the resolved type path", async () => {
    await withTempPmPath(async (context) => {
      ...
      expect(
        context.runCli(["schema", "add-type", "Spike", "--folder", "spikes"])
          .code,
      ).toBe(0);
  ...
  it("rejects invalid redaction patterns and item-id rewrites", async () => {
    await withTempPmPath(async (context) => {
      ...
      await expect(
        runHistoryRedact(id, { regex: "/[/" }, { path: context.pmPath }),
      ).rejects.toMatchObject<PmCliError>({
        exitCode: EXIT_CODE.USAGE,
  ...
  it("requires at least one matcher for history-redact", async () => {
    await withTempPmPath(async (context) => {
      ...
      await expect(
        runHistoryRedact(id, {}, { path: context.pmPath }),
      ).rejects.toMatchObject<PmCliError>({
        exitCode: EXIT_CODE.USAGE,
      });
    });
  ...
  it("fails history-redact for uninitialized, missing-item, missing-history, and empty-history paths", async () => {
    const tempDir = await mkdtemp(
      path.join(os.tmpdir(), "pm-history-redact-not-init-"),
    );
    try {
      await expect(
        runHistoryRedact("pm-missing", { literal: "x" }, { path: tempDir }),
      ).rejects.toMatchObject<PmCliError>({
        exitCode: EXIT_CODE.NOT_FOUND,
      });
    } finally {
    ...
    await withTempPmPath(async (context) => {
      ...
      await rm(historyPath, { force: true });
      await expect(
        runHistoryRedact(id, { literal: "x" }, { path: context.pmPath }),
      ).rejects.toMatchObject<PmCliError>({
        exitCode: EXIT_CODE.NOT_FOUND,
      });
  ...
  it("redacts via the history-stream fallback when the item file is absent (prefixed and bare ids)", async () => {
    await withTempPmPath(async (context) => {
      ...
      const prefixedId = createItem(
        context,
        "History Redact Fallback Prefixed",
      );
      context.runCli(
        [
          "append",
          ...
          "test-author",
          "--message",
          "append fallback token",
        ],
        { expectJson: true },
      );
      await rm(path.join(context.pmPath, "tasks", `${prefixedId}.toon`), {
        force: true,
      });
      ...
      const prefixedHistory = await readFile(
        path.join(context.pmPath, "history", `${prefixedId}.jsonl`),
        "utf8",
      );
      ...
      await rm(path.join(context.pmPath, "tasks", `${bareSourceId}.toon`), {
        force: true,
      });
  ...
  it("reports preexisting hash mismatches before successful redaction", async () => {
    await withTempPmPath(async (context) => {
      const id = createItem(
        context,
        "History Redact Preexisting Hash Mismatch",
      );
      const historyPath = path.join(context.pmPath, "history", `${id}.jsonl`);
      const lines = (await readFile(historyPath, "utf8"))
        .split("\n")
      ...
      expect(
        redacted.warnings.some((warning) =>
          warning.startsWith("history_redact_preexisting_hash_mismatches:"),
        ),
      ).toBe(true);
    });
  });
      ...
      try {
        await expect(
          ...
          message: expect.stringContaining(
            `History for ${id} changed while waiting for lock; retry history-redact.`,
          ),
        });
    ...
    await withTempPmPath(async (context) => {
      ...
      try {
        await expect(
          ...
          exitCode: EXIT_CODE.CONFLICT,
          message: expect.stringContaining(
            `Item ${id} changed while waiting for lock; retry history-redact.`,
          ),
        });
        const historyRaw = await readFile(historyFile, "utf8");
        expect(historyRaw).toContain(leakedToken);
  ...
  it("fails when history target item does not exist", async () => {
    await withTempPmPath(async (context) => {
      await expect(
        runHistory("pm-does-not-exist", {}, { path: context.pmPath }),
      ).rejects.toMatchObject<PmCliError>({
        exitCode: EXIT_CODE.NOT_FOUND,
      });
    });
  ...
  it("fails history and activity in strict mode when required streams are missing", async () => {
    await withTempPmPath(async (context) => {
      ...
      await rm(path.join(context.pmPath, "history", `${historyId}.jsonl`), {
        force: true,
      });
      await rm(path.join(context.pmPath, "history", `${activityId}.jsonl`), {
        force: true,
      });

      await expect(
        runHistory(historyId, {}, { path: context.pmPath }),
      ).rejects.toMatchObject<PmCliError>({
        exitCode: EXIT_CODE.NOT_FOUND,
      });
      await expect(
        runActivity({}, { path: context.pmPath }),
      ).rejects.toMatchObject<PmCliError>({
        exitCode: EXIT_CODE.NOT_FOUND,
      });
    });
  ...
  it("rejects malformed history lines", async () => {
    await withTempPmPath(async (context) => {
      ...
      await expect(
        readHistoryEntries(historyPath, id),
      ).rejects.toMatchObject<PmCliError>({
  ...
  it("aggregates activity across item history files deterministically", async () => {
    await withTempPmPath(async (context) => {
      ...
      expect(allActivity.activity.some((entry) => entry.id === secondId)).toBe(
        true,
      );
      expect(allActivity).toMatchObject({
        limit: 5,
        total_count: allActivity.count,
        omitted_count: 0,
        has_more: false,
        applied_bound: { kind: "limit", source: "default", value: 5 },
      });
      ...
      expect(limited).toMatchObject({
        ...
        has_more: true,
        applied_bound: { kind: "limit", source: "explicit", value: 1 },
      });
      ...
      expect(unbounded).toMatchObject({
        limit: null,
        total_count: unbounded.count,
        omitted_count: 0,
        has_more: false,
        applied_bound: { kind: "unbounded", source: "explicit", value: null },
      });

      const cliDefault = context.runCli(
        ["activity", "--json", "--limit", "5"],
        { expectJson: true },
      );
      expect(cliDefault.code).toBe(0);
      ...
      const cliFull = context.runCli(
        ["activity", "--json", "--full", "--limit", "5"],
        { expectJson: true },
      );
      expect(cliFull.code).toBe(0);
      expect(cliFull.json).toMatchObject({ compact: false });
      expect(
        (cliFull.json as { activity?: unknown[] }).activity?.length,
      ).toBeGreaterThan(0);
      ...
      expect(conflictingProjection.code).toBe(EXIT_CODE.USAGE);
      expect(conflictingProjection.stderr).toContain(
        "Activity projection options are mutually exclusive",
      );

      const conflictingBound = context.runCli([
        "activity",
        "--unbounded",
        "--limit",
        "1",
      ]);
      expect(conflictingBound.code).toBe(EXIT_CODE.USAGE);
      expect(conflictingBound.stderr).toContain(
        "Activity --unbounded cannot be combined with --limit",
      );
      await expect(
        runActivity({ unbounded: true, limit: "1" }, { path: context.pmPath }),
      ).rejects.toThrow("Activity --unbounded cannot be combined with --limit");
    });
  });
  ...
  it("covers activity pure helper edge cases", async () => {
    ...
    expect(activityInternals.parseNonEmptyFilter("  update  ", "--op")).toBe(
      "update",
    );
    expect(
      activityInternals.parseNonEmptyFilter(undefined, "--op"),
    ).toBeUndefined();
    expect(() => activityInternals.parseNonEmptyFilter("  ", "--op")).toThrow(
      "--op must not be empty",
    );
    expect(() =>
      activityInternals.parseRangeBound("  ", now, "--from"),
    ).toThrow("Activity time bounds must not be empty");
    expect(
      activityInternals.parseRangeBound(
    ...
    expect(
      activityInternals.parseRangeBound(undefined, now, "--from"),
    ).toBeUndefined();
    ...
    expect(
      activityInternals.includeByTimeWindow(
        { ...baseEntry, ts: "" },
        "2026-01-01T00:00:00.000Z",
        undefined,
      ),
    ...
    expect(
      activityInternals.includeByTimeWindow(
        { ...baseEntry, ts: "2026-01-01T00:00:00.000Z" },
    ...
    expect(
      activityInternals.includeByTimeWindow(
        { ...baseEntry, ts: "2026-01-03T00:00:00.000Z" },
        undefined,
        "2026-01-03T00:00:00.000Z",
      ),
    ).toBe(false);
    expect(activityInternals.limitEntries([1, 2, 3], undefined)).toEqual([
      1, 2, 3,
    ]);
    expect(activityInternals.limitEntries([1, 2, 3], 2)).toEqual([1, 2]);
    expect(activityInternals.readActivityString(42, "fallback")).toBe(
      "fallback",
    );
    expect(
      activityInternals.normalizeActivityEntry("pm-x", {
        op: 1,
        ts: 2,
        author: 3,
        patch: "bad",
    ...
    try {
      expect(
        await activityInternals.listHistoryFiles(path.join(tempDir, "missing")),
      ).toEqual([]);
      await writeFile(
        path.join(tempDir, "not-a-directory"),
        "not a directory",
        "utf8",
      );
      await expect(
        activityInternals.listHistoryFiles(
          path.join(tempDir, "not-a-directory"),
        ),
  ...
  it("sorts same-timestamp activity by id and operation", async () => {
    await withTempPmPath(async (context) => {
      ...
      expect(
        activity.activity.map((entry) => ({ id: entry.id, op: entry.op })),
  ...
  it("supports id/op/author/time-window filters for activity queries", async () => {
    await withTempPmPath(async (context) => {
      ...
      expect(byWindow.activity.map((entry) => entry.ts)).toEqual([
        "2026-01-02T00:00:00.000Z",
      ]);
    });
  });
  ...
  it("computes per-field before/after value diffs with --diff", async () => {
    await withTempPmPath(async (context) => {
      ...
      const updateEntry = diff.find((entry) =>
        entry.changes.some(
          (change) => change.field === "priority" && change.before === 1,
        ),
      );
      expect(updateEntry).toBeDefined();
      expect(
        updateEntry?.changes.find((change) => change.field === "priority"),
      ).toMatchObject({
        field: "priority",
        before: 1,
        after: 3,
    ...
    await withTempPmPath(async (context) => {
      ...
      await appendFile(
        ...
        `${JSON.stringify({
          ...
          patch: [
            { op: "move", from: "/metadata/goal", path: "/metadata/value" },
          ],
        })}\n`,
        "utf8",
      );
      ...
      expect(compact.compact_history?.[0]?.changed_fields).toEqual(
        expect.arrayContaining(["goal", "value"]),
      );
    });
  });
  ...
  it("restricts the diff to one field's transitions with --field (implying --diff)", async () => {
    await withTempPmPath(async (context) => {
      ...
      for (const entry of diff) {
        ...
        expect(
          entry.changes.every((change) => change.field === "priority"),
        ).toBe(true);
      }
      expect(
        diff.some((entry) =>
          entry.changes.some(
            (change) => change.before === 1 && change.after === 4,
          ),
        ),
      ).toBe(true);
    });
