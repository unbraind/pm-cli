...
beforeEach(() => {
  ...
  vi.mocked(runStartBackgroundRun).mockResolvedValue({
    run_id: "run-1",
  } as never);
  ...
  vi.mocked(runDuplicates).mockResolvedValue({
    count: 0,
    clusters: [],
  } as never);
  vi.mocked(runHealth).mockResolvedValue({ ok: true, warnings: [] } as never);
  vi.mocked(runValidate).mockResolvedValue({
    ok: true,
    has_warnings: false,
  } as never);
  ...
  vi.mocked(runFocus).mockResolvedValue({
    action: "set",
    focused_item: "pm-1",
  } as never);
  ...
  vi.mocked(runDelete).mockResolvedValue({
    id: "pm-1",
    dry_run: false,
  } as never);
  ...
  vi.mocked(runHistoryRedact).mockResolvedValue({
    id: "pm-1",
    changed: true,
    dry_run: false,
  } as never);
  vi.mocked(assertHistoryRepairTarget).mockReturnValue(undefined as never);
  vi.mocked(runHistoryRepair).mockResolvedValue({
    id: "pm-1",
    repaired: true,
  } as never);
  vi.mocked(runHistoryRepairAll).mockResolvedValue({
    totals: { failed: 0 },
  } as never);
  vi.mocked(runHistoryCompact).mockResolvedValue({
    id: "pm-1",
    compacted: true,
  } as never);
  vi.mocked(runSchemaAddType).mockResolvedValue({
    action: "add-type",
    warnings: [],
  } as never);
  vi.mocked(runSchemaRemoveType).mockResolvedValue({
    action: "remove-type",
    warnings: [],
  } as never);
  vi.mocked(runSchemaAddStatus).mockResolvedValue({
    action: "add-status",
    warnings: [],
  } as never);
  vi.mocked(runSchemaRemoveStatus).mockResolvedValue({
    action: "remove-status",
    warnings: [],
  } as never);
  vi.mocked(runSchemaAddField).mockResolvedValue({
    action: "add-field",
    warnings: [],
  } as never);
  vi.mocked(runSchemaRemoveField).mockResolvedValue({
    action: "remove-field",
    warnings: [],
  } as never);
  vi.mocked(runSchemaListFields).mockResolvedValue({
    action: "list-fields",
  } as never);
  vi.mocked(runSchemaShowField).mockResolvedValue({
    action: "show-field",
  } as never);
  vi.mocked(runSchemaApplyPreset).mockResolvedValue({
    action: "apply-preset",
    warnings: [],
  } as never);
  vi.mocked(runSchemaInferTypes).mockResolvedValue({
    action: "infer-types",
    warnings: [],
  } as never);
  ...
  vi.mocked(runSchemaShowStatus).mockResolvedValue({
    action: "show-status",
  } as never);
  vi.mocked(runComments).mockResolvedValue({
    id: "pm-1",
    comments: [],
  } as never);
  vi.mocked(runNotes).mockResolvedValue({ id: "pm-1", notes: [] } as never);
  vi.mocked(runLearnings).mockResolvedValue({
    id: "pm-1",
    learnings: [],
  } as never);
  vi.mocked(runFiles).mockResolvedValue({ id: "pm-1", files: [] } as never);
  vi.mocked(runFilesDiscover).mockResolvedValue({
    id: "pm-1",
    changed: true,
  } as never);
  ...
  vi.mocked(summarizeInitResult).mockReturnValue({
    initialized: true,
  } as never);
...
describe("register modules command surface", () => {
  it("registers the full built-in command surface across all register modules", () => {
    ...
    const context = program.commands.find(
      (command) => command.name() === "context",
    );
    expect(context?.aliases()).toContain("ctx");
    const packageCommand = program.commands.find(
  ...
  it("keeps hidden snake_case aliases parse-functional but out of help text", () => {
    ...
    const hidden = list?.options.find(
      (option) => option.long === "--assignee_filter",
    );
    expect(hidden).toBeDefined();
    expect(hidden?.hidden).toBe(true);
    const update = program.commands.find(
      (command) => command.name() === "update",
    );
    expect(
      update?.options.some(
        (option) => option.long === "--allow_ownership_bypass",
      ),
    ).toBe(false);
  });
  ...
  it("honors the list-query command filter including the ctx alias", () => {
    const program = new Command();
    registerListQueryCommands(program, {
      commandFilter: new Set(["ctx", "list-open"]),
    });
...
describe("list-query command actions", () => {
  it("defaults pm list to brief projection and terminal exclusion", async () => {
    await runCli("list");
    const options = lastCallArg<Record<string, unknown>>(
      vi.mocked(runList) as never,
      1,
    );
  ...
  it("rejects --stream without --json and streams NDJSON rows with --json", async () => {
    await expect(runCli("list", "--stream")).rejects.toThrow(
  ...
  it("renders list, search, and context --format ndjson as row-only streams", async () => {
    ...
    expect(stdout).toHaveBeenLastCalledWith('{"id":"pm-search","score":10}\n');
  ...
  it("normalizes aggregate options through the real helper", async () => {
    ...
    const options = lastCallArg<Record<string, unknown>>(
      vi.mocked(runAggregate) as never,
      0,
    );
  ...
  it("renders context markdown output and falls through to printResult otherwise", async () => {
    ...
    const normalized = lastCallArg<Record<string, unknown>>(
      vi.mocked(runContext) as never,
      0,
    );
  ...
  it("joins search keywords and resolves the search mode shorthands", async () => {
    ...
    let options = lastCallArg<Record<string, unknown>>(
    ...
    await expect(runCli("search", "   ")).rejects.toThrow(
  ...
  it("guards history projection conflicts and derives diff mode from --field", async () => {
    await expect(
      runCli("history", "pm-1", "--compact", "--full"),
  ...
  it("validates activity stream modes and streams entries with --json", async () => {
    ...
    const normalized = lastCallArg<Record<string, unknown>>(
      vi.mocked(runActivity) as never,
      0,
    );
...
describe("operation command actions", () => {
  it("requires either an explicit claim id or --next", () => {
    expect(() => requireClaimTarget(undefined, false)).toThrow(
      "Specify an item id or pass --next",
    );
    expect(() => requireClaimTarget("pm-a", true)).toThrow(
      "either an item id or --next",
    );
  ...
  it("routes claim --next through ranked atomic selection", async () => {
    ...
    vi.mocked(runClaimNext).mockResolvedValueOnce({
      id: "pm-next-defaults",
    } as never);
  ...
  it("routes pm test --background through the background run starter", async () => {
    ...
    const request = lastCallArg<Record<string, unknown>>(
      vi.mocked(runStartBackgroundRun) as never,
      0,
    );
  ...
  it("dispatches the hidden test-runs worker command", async () => {
    ...
    expect(vi.mocked(runTestRunsWorker)).toHaveBeenCalledWith(
      "run-42",
      expect.anything(),
    );
  });
  ...
  it("resolves telemetry subcommand routing including the legacy namespace", async () => {
    ...
    let options = lastCallArg<Record<string, unknown>>(
      vi.mocked(runTelemetry) as never,
      0,
    );
    ...
    await expect(runCli("telemetry", "status", "extra")).rejects.toThrow(
      "Unknown pm telemetry path",
    );
  });
  ...
  it("routes test/test-all background variants and omits profile timing when not requested", async () => {
    ...
    const testRequest = lastCallArg<Record<string, unknown>>(
      vi.mocked(runStartBackgroundRun) as never,
      0,
    );
    ...
    const testAllRequest = lastCallArg<Record<string, unknown>>(
      vi.mocked(runStartBackgroundRun) as never,
      0,
    );
  ...
  it("threads author/message and metadata-profile string options through operation handlers", async () => {
    ...
    const backgroundRequest = lastCallArg<Record<string, unknown>>(
      vi.mocked(runStartBackgroundRun) as never,
      0,
    );
    ...
    expect(
      lastCallArg<Record<string, unknown>>(vi.mocked(runValidate) as never, 0)
        .metadataProfile,
    ).toBe("strict");

    await runCli("claim", "pm-1", "--message", "claiming");
    expect(vi.mocked(runClaim)).toHaveBeenLastCalledWith(
      "pm-1",
      false,
      expect.anything(),
      {
    ...
    ).rejects.toThrow("conflicting --author and --assignee");
    ...
    expect(vi.mocked(runRelease)).toHaveBeenLastCalledWith(
      "pm-1",
      false,
      expect.anything(),
      {
    ...
    expect(
      lastCallArg<Record<string, unknown>>(vi.mocked(runUpdate) as never, 1)
        .message,
    ).toBe("begin");

    await runCli("start-task", "pm-1", "--assignee", "start-alias");
    expect(
      lastCallArg<Record<string, unknown>>(vi.mocked(runClaim) as never, 3)
        .author,
    ).toBe("start-alias");
    expect(
      lastCallArg<Record<string, unknown>>(vi.mocked(runUpdate) as never, 1)
    ...
    expect(
      lastCallArg<Record<string, unknown>>(vi.mocked(runRelease) as never, 3)
        .author,
    ).toBe("pause-alias");
  ...
  it("maps stats, gc, and contracts options", async () => {
    ...
    expect(
      lastCallArg<Record<string, unknown>>(vi.mocked(runStats) as never, 1)
        .storage,
    ).toBe(true);

    await runCli("gc", "--dry-run", "--scope", "locks", "--scope", "index");
    const gcOptions = lastCallArg<Record<string, unknown>>(
      vi.mocked(runGc) as never,
      1,
    );
    ...
    const contractsOptions = lastCallArg<Record<string, unknown>>(
      vi.mocked(runContracts) as never,
      0,
    );
  ...
  it("maps expanded operation option booleans and string payloads", async () => {
    ...
    const testOptions = lastCallArg<Record<string, unknown>>(
      vi.mocked(runTest) as never,
      1,
    );
    ...
    const testAllOptions = lastCallArg<Record<string, unknown>>(
      vi.mocked(runTestAll) as never,
      0,
    );
    ...
    const healthOptions = lastCallArg<Record<string, unknown>>(
      vi.mocked(runHealth) as never,
      1,
    );
    ...
    const validateOptions = lastCallArg<Record<string, unknown>>(
      vi.mocked(runValidate) as never,
      0,
    );
  ...
  it("maps claim/release options and refreshes search caches", async () => {
    ...
    expect(vi.mocked(runClaim)).toHaveBeenCalledWith(
      "pm-1",
      true,
      expect.anything(),
      {
    ...
    expect(vi.mocked(runRelease)).toHaveBeenCalledWith(
      "pm-1",
      true,
      expect.anything(),
      {
  ...
  it("composes lifecycle aliases from claim/update/close/release", async () => {
    ...
    let updateOptions = lastCallArg<Record<string, unknown>>(
      vi.mocked(runUpdate) as never,
      1,
    );
    expect(updateOptions.status).toBe("in_progress");

    await runCli("pause-task", "pm-1");
    updateOptions = lastCallArg<Record<string, unknown>>(
      vi.mocked(runUpdate) as never,
      1,
    );
...
describe("mutation command actions", () => {
  it("supports both create positional forms and guards bare type positionals", async () => {
    ...
    let normalized = lastCallArg<Record<string, unknown>>(
      vi.mocked(runCreate) as never,
      0,
    );
    ...
    await expect(runCli("create", "Epic")).rejects.toThrow(
      "looks like an item type",
    );
    expect(invalidateSearchCachesForMutation).toHaveBeenCalledTimes(2);
  });
  ...
  it("maps copy, update, and append option surfaces", async () => {
    ...
    const updateOptions = lastCallArg<Record<string, unknown>>(
      vi.mocked(runUpdate) as never,
      1,
    );
    ...
    expect(
      lastCallArg<Record<string, unknown>>(vi.mocked(runAppend) as never, 1)
        .body,
    ).toBe("more detail");
    await expect(runCli("append", "pm-1", "x", "--body", "y")).rejects.toThrow(
      "exactly one source",
    );
    await expect(runCli("append", "pm-1")).rejects.toThrow(
      "Missing append text",
    );
  });
  ...
  it("maps focus set/show/clear option surfaces", async () => {
    ...
    expect(vi.mocked(runFocus)).toHaveBeenCalledWith(
      "pm-1",
      { clear: false },
      expect.anything(),
    );
    ...
    await runCli("focus");
    expect(
      lastCallArg<string | undefined>(vi.mocked(runFocus) as never, 0),
    ).toBeUndefined();

    await runCli("focus", "--clear");
    expect(
      lastCallArg<Record<string, unknown>>(vi.mocked(runFocus) as never, 1)
        .clear,
    ).toBe(true);
  });
  ...
  it("maps bulk update/close filters and mutation payloads", async () => {
    ...
    const closeManyRequest = lastCallArg<Record<string, unknown>>(
      vi.mocked(runCloseMany) as never,
      0,
    );
  ...
  it("skips search-cache invalidation for delete dry runs", async () => {
    ...
    const deleteOptions = lastCallArg<Record<string, unknown>>(
      vi.mocked(runDelete) as never,
      1,
    );
  ...
  it("maps restore and deps options", async () => {
    ...
    const depsOptions = lastCallArg<Record<string, unknown>>(
      vi.mocked(runDeps) as never,
      1,
    );
    expect(depsOptions.format).toBe("context");
    expect(depsOptions.summary).toBe(true);
    expect(depsOptions.maxDepth).toBe("2");
    expect(depsOptions).toMatchObject({
      nodeLimit: "5",
      edgeLimit: "8",
      tokenBudget: "400",
      cursor: "next-page",
    ...
    const filteredDepsOptions = lastCallArg<Record<string, unknown>>(
      vi.mocked(runDeps) as never,
      1,
    );
  ...
  it("routes plan subcommands, aliases, positional titles, and reorder validation", async () => {
    await expect(runCli("plan")).rejects.toThrow(
      "pm plan requires a subcommand",
    );
    await expect(runCli("plan", "list")).rejects.toThrow(
      "Unknown pm plan subcommand",
    );
    ...
    await expect(
      runCli("plan", "reorder-step", "pm-plan", "step-1", "not-int"),
    ).rejects.toThrow("requires an integer new order");
    await runCli(
      "plan",
      "reorder-step",
      "pm-plan",
  ...
  it("routes history redaction, repair, repair-all, and compaction actions", async () => {
    ...
    vi.mocked(runHistoryRedact).mockResolvedValueOnce({
      id: "pm-1",
      changed: true,
      dry_run: true,
    } as never);
    ...
    expect(vi.mocked(assertHistoryRepairTarget)).toHaveBeenCalledWith(
      "pm-1",
      false,
    );
    ...
    vi.mocked(runHistoryRepairAll).mockResolvedValueOnce({
      totals: { failed: 2 },
    } as never);
  ...
  it("routes schema subcommands, shorthand add-type, aliases, warnings, and JSON output", async () => {
    await expect(runCli("schema")).rejects.toThrow(
      "pm schema requires a subcommand",
    );
    await expect(runCli("schema", "bogus", "Name")).rejects.toThrow(
      "Unknown pm schema subcommand",
    );
    await expect(runCli("schema", "list-statuses")).rejects.toThrow(
      "Unknown pm schema subcommand",
    );
    await expect(runCli("schema", "help")).rejects.toThrow(
      "Unknown pm schema subcommand",
    );
    ...
    expect(vi.mocked(runSchemaShow)).toHaveBeenCalledWith(
      "Task",
      expect.anything(),
    );
    ...
    expect(vi.mocked(runSchemaShowStatus)).toHaveBeenCalledWith(
      "open",
      expect.anything(),
    );
    ...
    vi.mocked(runSchemaAddStatus).mockResolvedValueOnce({
      action: "add-status",
      warnings: ["hook:warn"],
    } as never);
    await runCliRaw(
    ...
    expect(vi.mocked(formatSchemaListHuman)).toHaveBeenCalledTimes(1);
    await expect(
      runCli("schema", "add-status", "bad", "--order", "not-a-number"),
    ).rejects.toThrow("--order must be a finite integer");
    ...
    expect(vi.mocked(runSchemaShowField)).toHaveBeenCalledWith(
      "severity_level",
      expect.anything(),
    );
  ...
  it("routes profile subcommands, human/JSON rendering, apply warnings, and usage errors", async () => {
    await expect(runCli("profile")).rejects.toThrow(
      "pm profile requires a subcommand",
    );
    await expect(runCli("profile", "frobnicate")).rejects.toThrow(
      "Unknown pm profile subcommand",
    );
    ...
    vi.mocked(runProfileApply).mockResolvedValueOnce({
      action: "apply",
      warnings: ["hook:warn"],
    } as never);
  ...
  it("guards annotation text sources and skips refresh for read-only listings", async () => {
    ...
    const editArgs = lastCallArg<Record<string, unknown>>(
      vi.mocked(runComments) as never,
      1,
    );
    ...
    await expect(runCli("notes", "pm-1", "a", "--add", "b")).rejects.toThrow(
      "not both",
    );

    await runCli("learnings", "pm-1", "lesson", "--force");
    const learningsOptions = lastCallArg<Record<string, unknown>>(
  ...
  it("maps files/docs link management and discover routing", async () => {
    ...
    const filesOptions = lastCallArg<Record<string, unknown>>(
      vi.mocked(runFiles) as never,
      1,
    );
    ...
    const discoverOptions = lastCallArg<Record<string, unknown>>(
      vi.mocked(runFilesDiscover) as never,
      1,
    );
    ...
    expect(
      lastCallArg<Record<string, unknown>>(vi.mocked(runDocs) as never, 1).list,
    ).toBe(true);
    expect(invalidateSearchCachesForMutation).toHaveBeenCalledTimes(2);
  });
  ...
  it("routes docs mutations and close fallback fields", async () => {
    ...
    const closeOptions = lastCallArg<Record<string, unknown>>(
      vi.mocked(runClose) as never,
      2,
    );
    ...
    const docsOptions = lastCallArg<Record<string, unknown>>(
      vi.mocked(runDocs) as never,
      1,
    );
  ...
  it("suggests list alternatives for a plan ls/list subcommand", async () => {
    await expect(runCli("plan", "ls")).rejects.toThrow(
      "Unknown pm plan subcommand",
    );
  ...
  it("coerces a positive 1-based integer option and rejects non-positive/non-integer values", () => {
    ...
    expect(() => parse("0")).toThrow(
      "--edit must be a positive integer (1-based index).",
    );
  ...
  it("resolves --body-file content for create and update and rejects --body + --body-file", async () => {
    ...
    try {
      ...
      const createNormalized = lastCallArg<Record<string, unknown>>(
        vi.mocked(runCreate) as never,
        0,
      );
      ...
      await expect(
        runCli("update", "pm-1", "--body", "inline", "--body-file", bodyPath),
      ).rejects.toThrow("mutually exclusive");
    } finally {
  ...
  it("forwards copy/delete/append/restore author and message option values", async () => {
    ...
    const appendOptions = lastCallArg<Record<string, unknown>>(
      vi.mocked(runAppend) as never,
      1,
    );
    expect(appendOptions).toMatchObject({
      body: "appended via alias",
      author: "agent",
      message: "note",
    });
  ...
  it("maps the full update-many and close-many filter/mutation surfaces", async () => {
    await runCli(
      ...
      "--filter-priority",
      "1",
      "--filter-deadline-before",
      "2026-12-31",
      "--filter-deadline-after",
      "2026-01-01",
    ...
    const umRequest = lastCallArg<Record<string, unknown>>(
      vi.mocked(runUpdateMany) as never,
      0,
    );
    ...
    const cmRequest = lastCallArg<Record<string, unknown>>(
      vi.mocked(runCloseMany) as never,
      0,
    );
  ...
  it("maps every bulk content + governance filter flag to its selector field (true branch)", async () => {
    ...
    const presenceList = lastCallArg<Record<string, unknown>>(
      vi.mocked(runUpdateMany) as never,
      0,
    ...
    const absenceList = lastCallArg<Record<string, unknown>>(
      vi.mocked(runCloseMany) as never,
      0,
    ).list as Record<string, unknown>;
  ...
  it("rejects a both-present-and-absent bulk content filter with the --filter-* flag names", async () => {
    ...
    await expect(
      ...
      "Cannot combine --filter-has-notes with --filter-no-notes for the same field.",
    );
  ...
  it("covers remaining close/close-many/history/files-discover option arms", async () => {
    ...
    expect(
      lastCallArg<Record<string, unknown>>(vi.mocked(runClose) as never, 2)
        .validateClose,
    ).toBe("warn");

    // close-many with --dry-run and no reason.
    await runCli("close-many", "--filter-status", "open", "--dry-run");
    const cm = lastCallArg<Record<string, unknown>>(
      vi.mocked(runCloseMany) as never,
      0,
    );
    ...
    const redact = lastCallArg<Record<string, unknown>>(
      vi.mocked(runHistoryRedact) as never,
      1,
    );
    ...
    expect(
      lastCallArg<Record<string, unknown>>(
        vi.mocked(runHistoryRepair) as never,
        1,
      ).author,
    ).toBe("agent");
    ...
    const compact = lastCallArg<Record<string, unknown>>(
      vi.mocked(runHistoryCompact) as never,
      1,
    );
    ...
    const discover = lastCallArg<Record<string, unknown>>(
      vi.mocked(runFilesDiscover) as never,
      1,
    );
    ...
    expect(
      lastCallArg<Record<string, unknown>>(vi.mocked(runUpdateMany) as never, 0)
        .status,
    ).toBeUndefined();
    ...
    await expect(runCli("plan", "frobnicate")).rejects.toThrow(
      "Unknown pm plan subcommand",
    );
  });

  it("routes history-compact bulk selectors and validates their inputs", async () => {
    vi.mocked(runHistoryCompactBulk).mockResolvedValue({
      totals: { items_errored: 0 },
    } as never);

    // --ids → bulk mode with a parsed id list (no positional id).
    await runCli("history-compact", "--ids", "pm-1,pm-2", "--dry-run");
    expect(vi.mocked(assertHistoryCompactTarget)).toHaveBeenLastCalledWith(
      undefined,
      {
        ids: ["pm-1", "pm-2"],
        allOver: undefined,
    ...
    expect(
      lastCallArg<Record<string, unknown>>(
        vi.mocked(runHistoryCompactBulk) as never,
        0,
      ),
    ).toMatchObject({
      ids: ["pm-1", "pm-2"],
    ...
    vi.mocked(runHistoryCompactBulk).mockResolvedValueOnce({
      totals: { items_errored: 2 },
    } as never);
    ...
    await expect(
      runCli("history-compact", "--closed", "--all-streams"),
    ).rejects.toThrow(/mutually exclusive/);

    // --before is rejected in bulk mode.
    await expect(
      runCli("history-compact", "--all-streams", "--before", "5"),
    ).rejects.toThrow(/--before applies only in single-id mode/);

    // --all-over / --min-entries reject negative, non-numeric, and truncating-float inputs.
    await expect(runCli("history-compact", "--all-over", "-3")).rejects.toThrow(
      /--all-over/,
    );
    await expect(
      runCli("history-compact", "--all-over", "notanumber"),
    ).rejects.toThrow(/--all-over/);
    await expect(
      runCli("history-compact", "--all-over", "3.5"),
    ).rejects.toThrow(/--all-over/);
    await expect(
      runCli("history-compact", "--min-entries", "-1"),
    ).rejects.toThrow(/--min-entries/);
    await expect(
      runCli("history-compact", "--min-entries", "10abc"),
    ).rejects.toThrow(/--min-entries/);
  });
  ...
  it("maps full schema add-status/add-type options and string-form alias/role inputs", async () => {
    ...
    const bareAddType = lastCallArg<Record<string, unknown>>(
      vi.mocked(runSchemaAddType) as never,
      1,
    );
  ...
  it("preserves defensive string coercion for programmatic schema alias and role options", async () => {
    ...
    const schemaCommand = program.commands.find(
      (candidate) => candidate.name() === "schema",
    );
    if (!schemaCommand) {
  ...
  it("rejects read-like unknown schema subcommands instead of add-type shorthand mutation (GH-293)", async () => {
    for (const token of [
      ...
      await expect(runCliRaw("schema", token)).rejects.toThrow(
        `Unknown pm schema subcommand "${token}"`,
      );
    }
    expect(vi.mocked(runSchemaAddType)).not.toHaveBeenCalledWith(
      expect.stringMatching(
        /^(list-types|list-statuses|show-all|show-statuses)$/,
      ),
      expect.anything(),
      expect.anything(),
    );
  ...
  it("maps full comments/notes/learnings/files/docs option surfaces", async () => {
    ...
    const commentsOptions = lastCallArg<Record<string, unknown>>(
      vi.mocked(runComments) as never,
      1,
    );
    ...
    expect(docsOptions).toMatchObject({
      note: "doc note",
      author: "agent",
      message: "docs",
      force: true,
    ...
    expect(
      lastCallArg<Record<string, unknown>>(vi.mocked(runDeps) as never, 1),
    ).toMatchObject({
      collapse: "repeated",
      maxDepth: "3",
    });
  ...
  it("runs every mutation command without --profile and skips refresh for read-only listings", async () => {
    ...
    vi.mocked(runFilesDiscover).mockResolvedValueOnce({
      id: "pm-1",
      changed: false,
    } as never);
...
describe("setup command actions", () => {
  ...
  it("renders describe output as Markdown to stdout or a file when --markdown is set", async () => {
    ...
    const outputDir = await mkdtemp(
      path.join(tmpdir(), "pm-describe-markdown-output-"),
    );
    const outputPath = path.join(outputDir, "docs", "pm-x-reference.md");
    const outputStdout = vi
    ...
    try {
      ...
      try {
        await runCliRaw(
          ...
          "--markdown",
          "--output",
          path.join(outputDir, "pm-x-visible.md"),
        );
        expect(outputStdout.mock.calls.length).toBe(0);
        expect(
          outputStderr.mock.calls.map((call) => String(call[0])).join(""),
        ).toContain("warning: pm-x failed to load on the global layer");
      } finally {
  ...
  it("rejects --markdown combined with --json", async () => {
    await expect(
      runCliRaw("package", "describe", "--markdown", "--json"),
    ).rejects.toThrow("Cannot combine --json with --markdown");
  });
  ...
  it("rejects --markdown for a non-describe action", async () => {
    ...
    await expect(
      runCliRaw("package", "--manage", "--markdown"),
    ).rejects.toThrow("--markdown is only supported by the describe action");
  });

  it("rejects --output without markdown or a non-empty path", async () => {
    vi.mocked(runExtension).mockClear();
    await expect(
      runCliRaw("package", "describe", "pm-x", "--output", "docs/pm-x.md"),
    ).rejects.toThrow(
    ...
    await expect(
      runCliRaw("package", "describe", "pm-x", "--markdown", "--output", "   "),
    ).rejects.toThrow("--output requires a non-empty file path");
    expect(vi.mocked(runExtension)).not.toHaveBeenCalled();
  });
  ...
  it("installs multiple explicit package targets while preserving shell-expanded wildcard recovery", async () => {
    ...
    expect(
      lastCallArg<Record<string, unknown>>(vi.mocked(runExtension) as never, 1),
    ).toMatchObject({
      install: true,
      vocabulary: "package",
    });
    ...
    const wildcardDir = await mkdtemp(
      path.join(tmpdir(), "pm-register-wildcard-"),
    );
  ...
  it("continues multi-target install after target failures and marks the aggregate failed", async () => {
    vi.mocked(runExtension)
      .mockResolvedValueOnce({
        ...
        action: "install",
        details: {
          extension: { name: "pkg-a" },
          destination_path: "/tmp/pkg-a",
        },
      ...
      .mockResolvedValueOnce({
        ...
        action: "install",
        details: {
          extension: { name: "pkg-b" },
          destination_path: "/tmp/pkg-b",
        },
      ...
      .mockRejectedValueOnce(
        new PmCliError("local source missing", EXIT_CODE.NOT_FOUND),
      )
  ...
  it("rejects multi-target install with forced GitHub sources before dispatch", async () => {
    vi.mocked(runExtension).mockClear();
    await expect(
      runCliRaw("install", "pkg-a", "pkg-b", "--github", "owner/repo"),
    ...
    await expect(
      runCliRaw("install", "pkg-a", "pkg-b", "--gh", "owner/repo"),
    ).rejects.toMatchObject<PmCliError>({
      exitCode: EXIT_CODE.USAGE,
      context: expect.objectContaining({
        code: "multi_target_github_install_ambiguous",
