...
describe("generateBashScript", () => {
  ...
  it("exposes universal read-output controls in every shell", () => {
    const expected = [
      "output-include",
      "output-limit",
      "output-budget",
    ...
    for (const flag of expected) {
      ...
      expect(generateZshScript()).toContain(`--${flag}`);
      expect(generateFishScript()).toContain(`-l ${flag}`);
    }
  });
  ...
  it("includes strict health flags in bash completion", () => {
    const script = generateBashScript();
    expect(script).toContain(
      "--strict-directories --require-merge-drivers --strict-exit --fail-on-warn --check-only --check-telemetry --no-refresh --refresh-vectors --verbose-stale-items --verbose-author-events --brief --summary --skip-vectors --skip-integrity --skip-drift --full --json --lean --token-accounting --output-include --output-limit --output-budget --output-format --output-session --output-cursor --output-row-contract --quiet --no-changed-fields --full-changed-fields --id-only --pm-path --path --no-extensions --no-pager --profile --help",
    );
  });
  ...
  it("includes activity filtering, raw projection, and stream contracts across shells", () => {
    ...
    expect(zsh).toContain("--raw[Emit raw compact per-event activity output]");
