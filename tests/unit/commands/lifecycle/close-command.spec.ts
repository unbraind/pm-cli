...
import {
  ...
  runCloseMany,
} from "../../../../src/cli/commands/lifecycle/close-many.js";
import {
  _testOnlyCloseCommand,
  runClose,
} from "../../../../src/cli/commands/lifecycle/close.js";
...
describe("runCloseMany via CLI", () => {
  it("fails when close-many tracker is not initialized", async () => {
    ...
    try {
      const { runCloseMany } =
        await import("../../../../src/cli/commands/lifecycle/close-many.js");
      await expect(
        runCloseMany(
          {
            list: { ids: "pm-missing" },
  ...
  it("rejects null and empty programmatic filters instead of matching every item", async () => {
    await withTempPmPath(async (context) => {
      ...
      await expect(
        import("../../../../src/cli/commands/lifecycle/close-many.js").then(
          ({ runCloseMany }) =>
            runCloseMany(
              {
