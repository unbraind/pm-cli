...
import { _testOnlyUpdateCommand, runUpdate, type UpdateCommandOptions } from "../../../../src/cli/commands/lifecycle/update.js";
    ...
    await withTempPmPath(async (context) => {
      ...
      const result = await runUpdate(
        id,
        {
          ...
          file: ["path=src/cli/commands/lifecycle/update.ts,scope=project,note=audit file"],
      ...
      expect(item.files).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ path: "src/cli/commands/lifecycle/update.ts", note: "audit file" }),
        ]),
      );
