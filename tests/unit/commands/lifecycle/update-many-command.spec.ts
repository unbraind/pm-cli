...
import {
  ...
  runUpdateMany,
} from "../../../../src/cli/commands/lifecycle/update-many.js";
import {
  checkpointFilePath,
  createCheckpointId,
...
describe("update-many command helper coverage", () => {
  it("normalizes summary, comparable values, collections, and tag planning", () => {
    ...
    const emptyRuntimeRegistry = {
      definitions: [],
      by_key: new Map(),
      by_cli_token: new Map(),
      command_to_fields: new Map(),
    };
    const noUpdateDiff = _testOnlyUpdateManyCommand.buildPlannedItemDiff(
      {
        id: "pm-preview",
        title: "preview",
        status: "open",
    ...
    // Non-array mutation inputs should be treated as no-op counts.
    expect(
      _testOnlyUpdateManyCommand.buildCollectionMutationPlans(
        { comments: ["old"] },
        { comment: "not-an-array" as never, depRemove: "pm-old" as never },
      ),
    ).toEqual([]);
  });
});
...
describe("runUpdateMany", () => {
  ...
  it("applies updates with the default synthesized message when --message is omitted", async () => {
    await withTempPmPath(async (context) => {
      ...
      expect(getItemPriority(context, id)).toBe(2);
    });
  });
  ...
  it("uses the default apply message when update.message is omitted", async () => {
    await withTempPmPath(async (context) => {
      const id = createTask(context, "bulk-default-message", {
        tags: "bulk-default-message",
        description: "before default message apply",
      });
      ...
      const result = await runUpdateMany(
        {
          list: { ids: id },
          update: {
            description: "after default message apply",
          },
        },
        { path: context.pmPath },
      );
