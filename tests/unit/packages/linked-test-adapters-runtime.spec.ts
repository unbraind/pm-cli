...
describe("linked-test adapters static SDK runtime", () => {
  ...
  it("normalizes options and delegates every adapter", async () => {
    ...
    const list = vi
      .spyOn(sdk, "runTestRunsList")
      .mockResolvedValue({ kind: "list" } as never);
    const status = vi
      .spyOn(sdk, "runTestRunsStatus")
      .mockResolvedValue({ kind: "status" } as never);
    const logs = vi
      .spyOn(sdk, "runTestRunsLogs")
      .mockResolvedValue({ kind: "logs" } as never);
    const stop = vi
      .spyOn(sdk, "runTestRunsStop")
      .mockResolvedValue({ kind: "stop" } as never);
    const resume = vi
      .spyOn(sdk, "runTestRunsResume")
      .mockResolvedValue({ kind: "resume" } as never);
    ...
    await runTestRunsLogsPackage(
      ["run-2"],
      { stream: "stderr", tail: "20" },
      global,
    );
  ...
  it("rejects missing run identifiers for every positional adapter", async () => {
    await expect(runTestRunsStatusPackage([], {} as never)).rejects.toThrow(
      "test-runs status requires a runId",
    );
    await expect(
      runTestRunsLogsPackage(["   "], {}, {} as never),
    ).rejects.toThrow("test-runs logs requires a runId");
    await expect(runTestRunsStopPackage([], {}, {} as never)).rejects.toThrow(
      "test-runs stop requires a runId",
    );
    await expect(runTestRunsResumePackage([], {} as never)).rejects.toThrow(
      "test-runs resume requires a runId",
    );
  });
});
