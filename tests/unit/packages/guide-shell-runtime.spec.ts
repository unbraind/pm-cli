...
describe("guide-shell static SDK runtime", () => {
  ...
  it("normalizes guide and completion inputs through public SDK functions", async () => {
    const guide = vi
      .spyOn(sdk, "runGuide")
      .mockResolvedValue({ topic: "sdk" } as never);
    await runGuidePackage(
      [" sdk "],
      { list: true, format: "json", depth: "deep" },
      { path: "/tmp/pm" },
    );
    ...
    const completion = vi
      .spyOn(sdk, "runCompletion")
      .mockReturnValue({ script: "complete" } as never);
    await runCompletionPackage(
      [" zsh "],
      { itemTypes: "Task,Issue", tags: "one,two", eagerTags: true },
      {},
    );
    ...
    await runCompletionPackage(
      [],
      { shell: "fish", item_types: "Feature", eager_tags: false },
      {},
    );
  ...
  it("builds completion registries and list helpers across schema shapes", async () => {
    ...
    const completion = vi
      .spyOn(sdk, "runCompletion")
      .mockReturnValue({ script: "ok" } as never);
    ...
    await expect(
      runCompletionTagsPackage({ path: "/tmp/pm" }),
    ).resolves.toEqual({
      tags: ["alpha", "beta"],
      count: 2,
    });
    await expect(
      runCompletionStatusesPackage({ path: "/tmp/pm" }),
    ).resolves.toEqual({
      statuses: ["closed", "open"],
      count: 2,
    });
    await expect(
      runCompletionTypesPackage({ path: "/tmp/pm" }),
    ).resolves.toEqual({
      types: ["Task"],
      count: 1,
    });
    ...
    vi.mocked(sdk.resolveRuntimeStatusRegistry).mockReturnValue({
      definitions: [],
    } as never);
    ...
    await expect(
      runCompletionTagsPackage({ path: "/tmp/missing" }),
    ).resolves.toEqual({
      tags: [],
      count: 0,
    });
    await expect(
      runCompletionStatusesPackage({ path: "/tmp/missing" }),
    ).resolves.toEqual({
      statuses: [],
      count: 0,
    });
    await expect(
      runCompletionTypesPackage({ path: "/tmp/missing" }),
    ).resolves.toEqual({
      types: [],
      count: 0,
    });
  ...
  it("renders guide, completion, and word-list result families", () => {
    const renderGuide = vi
      .spyOn(sdk, "renderGuideMarkdown")
    ...
    format.mockReturnValueOnce("toon" as never);
    expect(
      renderGuideShellPackageOutput({ command: "guide", payload: {} } as never),
    ).toBeNull();
