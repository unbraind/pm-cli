...
describe("eval reciprocalRankAtK", () => {
  it("returns the reciprocal of the first relevant rank within the cutoff", () => {
    expect(reciprocalRankAtK(["x", "a", "b"], new Set(["a"]), 10)).toBeCloseTo(
      1 / 2,
      12,
    );
...
describe("eval precisionAtK / recallAtK", () => {
  it("computes precision over the cutoff and recall over the relevant set", () => {
    expect(precisionAtK(["a", "x", "b"], new Set(["a", "b"]), 2)).toBeCloseTo(
      0.5,
      12,
    );
    expect(recallAtK(["a", "x", "b"], new Set(["a", "b", "c"]), 3)).toBeCloseTo(
      2 / 3,
      12,
    );
  });
...
describe("eval aggregateEvalMetrics", () => {
  it("returns zeros with query_count 0 for an empty input", () => {
    expect(aggregateEvalMetrics([])).toEqual({
      ndcg: 0,
      mrr: 0,
      precision: 0,
      recall: 0,
...
describe("eval parseEvalQuerySet", () => {
  it("publishes one self-describing contract and copy-pasteable example", () => {
    expect(EVAL_QUERY_SET_SCHEMA_ID).toBe(
      "https://schema.unbrained.dev/pm/eval-query-set/v1",
    );
    expect(EVAL_QUERY_SET_CONTRACT).toMatchObject({
      schema: EVAL_QUERY_SET_SCHEMA_ID,
      accepted_top_level: ["array", "object"],
      required_query_fields: ["query", "relevant_ids"],
      modes: ["keyword", "semantic", "hybrid"],
    });
    expect(parseEvalQuerySet(EVAL_QUERY_SET_EXAMPLE)).toMatchObject({
      schema: EVAL_QUERY_SET_SCHEMA_ID,
      queries: [
        expect.objectContaining({
          query: expect.any(String),
          relevant_ids: [expect.any(String)],
        }),
      ],
    });
  });
  ...
  it("accepts an object with a queries array and a per-query mode", () => {
    const parsed = parseEvalQuerySet({
      schema: EVAL_QUERY_SET_SCHEMA_ID,
      queries: [{ query: "x", relevant_ids: ["pm-1"], mode: "hybrid" }],
    });
    expect(parsed.schema).toBe(EVAL_QUERY_SET_SCHEMA_ID);
    expect(parsed.queries[0].mode).toBe("hybrid");
  });

  it("rejects an unsupported query-set schema", () => {
    expect(() =>
      parseEvalQuerySet({
        schema: "https://schema.unbrained.dev/pm/eval-query-set/v2",
        queries: [{ query: "x", relevant_ids: ["pm-1"] }],
      }),
    ).toThrow(/Unsupported eval query set schema/);
  });

  it("omits an empty/whitespace description", () => {
    const parsed = parseEvalQuerySet([
      { query: "x", relevant_ids: ["pm-1"], description: "   " },
    ]);
  ...
  it("rejects a non-object entry", () => {
    expect(() => parseEvalQuerySet([null])).toThrow(
  ...
  it("rejects a missing/empty/non-string query", () => {
    expect(() =>
      parseEvalQuerySet([{ query: "  ", relevant_ids: ["pm-1"] }]),
    ).toThrow(/non-empty "query"/);
    expect(() => parseEvalQuerySet([{ relevant_ids: ["pm-1"] }])).toThrow(
      /non-empty "query"/,
    );
  });

  it("rejects a missing relevant_ids array", () => {
    expect(() => parseEvalQuerySet([{ query: "x" }])).toThrow(
      /"relevant_ids" array/,
    );
  });
  ...
  it("rejects an empty relevant_ids list (after trimming)", () => {
    expect(() =>
      parseEvalQuerySet([{ query: "x", relevant_ids: ["", "  "] }]),
    ).toThrow(/at least one relevant id/);
  });

  it("rejects a relevant_ids array containing a non-string entry", () => {
    expect(() =>
      parseEvalQuerySet([{ query: "x", relevant_ids: ["pm-1", 7] }]),
  ...
  it("rejects an invalid mode", () => {
    expect(() =>
      parseEvalQuerySet([
        { query: "x", relevant_ids: ["pm-1"], mode: "fuzzy" },
      ]),
    ).toThrow(/invalid mode/);
    expect(() =>
      parseEvalQuerySet([{ query: "x", relevant_ids: ["pm-1"], mode: 7 }]),
    ).toThrow(/invalid mode/);
  });
