...
describe("resolveSettingsWithSemanticRuntimeDefaults", () => {
  ...
  it("treats PM_DISABLE_OLLAMA_AUTO_DEFAULTS=yes as disabled", () => {
    const settings = makeSettings();
    process.env.PM_DISABLE_OLLAMA_AUTO_DEFAULTS = "YES";
    const resolved = resolveSettingsWithSemanticRuntimeDefaults(settings);
    expect(resolved.auto_ollama_defaults_applied).toBe(false);
    expect(spawnSyncMock).not.toHaveBeenCalled();
  });

  it("treats PM_DISABLE_OLLAMA_AUTO_DEFAULTS=1 as disabled", () => {
    const settings = makeSettings();
    process.env.PM_DISABLE_OLLAMA_AUTO_DEFAULTS = "1";
    const resolved = resolveSettingsWithSemanticRuntimeDefaults(settings);
    expect(resolved.auto_ollama_defaults_applied).toBe(false);
  ...
  it("uses the first listed embedding model when no qwen embedding model exists", () => {
    ...
    spawnSyncMock.mockImplementation((_command: string, args: string[]) => {
      ...
      if (args[0] === "list") {
        return {
          status: 0,
          stdout: "NAME ID SIZE MODIFIED\nnomic-embed-text:latest abc 300 MB now\nllama3.2:latest def 2 GB now\n",
          stderr: "",
        };
      }
      return { status: 1, stdout: "", stderr: "" };
    ...
    const resolved = resolveSettingsWithSemanticRuntimeDefaults(settings);
    expect(resolved.auto_ollama_defaults_applied).toBe(true);
    expect(resolved.settings.providers.ollama.model).toBe("nomic-embed-text:latest");
  ...
  it("does not auto-apply when `ollama list` succeeds with non-string stdout", () => {
    ...
    spawnSyncMock.mockImplementation((_command: string, args: string[]) => {
      ...
      if (args[0] === "list") {
        return { status: 0, stdout: undefined, stderr: "" };
      }
      return { status: 1, stdout: "", stderr: "" };
    });
  ...
  it("fills only missing base_url when model and embedding model already exist", () => {
    ...
    settings.providers.ollama.model = "nomic-embed-text:latest";
    settings.vector_store.adapter = "lancedb";
    settings.vector_store.lancedb.path = "/tmp/custom-lancedb";
    settings.search.provider = "ollama";
    settings.search.embedding_model = "nomic-embed-text:latest";
  ...
  it("does not apply defaults when a non-ollama search provider is configured", () => {
    const settings = makeSettings();
