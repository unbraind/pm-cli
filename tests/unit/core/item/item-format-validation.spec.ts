...
describe("item-format metadata validation", () => {
  ...
  it("covers unknown-field sorting and plan-step non-object normalization branches", () => {
    ...
    expect(_testOnlyItemFormat.firstNonZeroComparison([0, -1, 1])).toBe(-1);
  });

  it("covers item-format internal normalization fallback branches", () => {
    expect(
      _testOnlyItemFormat.normalizeTestRunSummaries([
        {
          run_id: 1,
          kind: "bogus",
          status: "bogus",
