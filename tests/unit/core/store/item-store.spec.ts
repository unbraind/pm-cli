...
describe("core/store/item-store", () => {
  ...
  it("does not attempt rollback writes when skip_write/skip_delete is true and history append fails", async () => {
    await withTempPmPath(async ({ pmPath }) => {
      const id = "pm-skip-rollback-guards";
      const { itemPath } = await writeTaskItem(pmPath, id, { description: "before skip rollback guards" }, "toon");
      const originalRaw = await fs.readFile(itemPath, "utf8");
      const settings = await readSettings(pmPath);
      ...
      setActiveExtensionServices({
        overrides: [
          {
            layer: "project",
            name: "skip-write-fail-history",
            service: "item_store_write",
            run: () => ({ skip_write: true }),
          },
          {
            layer: "project",
            name: "skip-delete-fail-history",
            service: "item_store_delete",
            run: () => ({ skip_delete: true }),
          },
        ],
      ...
      await expect(
        mutateItem({
          ...
          author: "unit-author",
          mutate: (document) => {
            document.metadata.description = "skip write then fail history";
            return { changedFields: ["description"] };
          },
        }),
      ).rejects.toBeInstanceOf(Error);
