...
describe("history record integrity", () => {
  ...
  it("fails closed on malformed, sparse, or contradictory rewrite evidence", () => {
    ...
    const malformedEvidence: unknown[] = [
      ...
      { ...evidence, patch: [{ op: "add", path: "/title" }] },
      { ...evidence, patch: [{ op: "replace", path: "/title" }] },
      { ...evidence, patch: [{ op: "test", path: "/title" }] },
      { ...evidence, patch: [{ op: "move", path: "/title" }] },
      { ...evidence, patch: [{ op: "copy", path: "/title" }] },
