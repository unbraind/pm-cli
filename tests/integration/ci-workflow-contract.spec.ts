...
describe("GitHub workflow contract", () => {
  ...
  it("keeps release workflow aligned with tag-trigger npm publish contract", async () => {
    ...
    expectContainsAll(releaseWorkflow, [
      ...
      "run: pnpm smoke:npx",
      "fetch-depth: 0",
      "run: node scripts/generate-release-notes.mjs --version \"${GITHUB_REF_NAME#v}\" --output \"$RUNNER_TEMP/release-notes.md\"",
      ...
      "npx verification did not converge within retries.",
      "bunx verification did not converge within retries.",
