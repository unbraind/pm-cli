...
describe("init tracker-path guardrails", () => {
  it("preserves workspace discovery for active-host SDK init without an explicit path", async () => {
    await withTempPmPath(async (context) => {
      const workspaceRoot = path.join(context.tempRoot, "sdk-discovered-workspace");
      ...
      try {
        ...
        expect(initialized.next_steps.every((step) => !step.includes("--pm-path"))).toBe(
          true,
        );
      } finally {
  ...
  it("accepts flag-oriented id prefixes, preserves positional parity, and rejects conflicting inputs", async () => {
    await withTempPmPath(async (context) => {
      ...
      expect(flagged.json.settings).toMatchObject({ id_prefix: "ops-" });

      const aliasRoot = path.join(context.tempRoot, "alias-prefix");
      const aliased = context.runCli(
        ["--pm-path", aliasRoot, "init", "--prefix", "svc", "--json", "--yes"],
        { expectJson: true },
      );
      expect(aliased.code).toBe(0);
      expect(aliased.json.settings).toMatchObject({ id_prefix: "svc-" });

      const positionalRoot = path.join(context.tempRoot, "positional-prefix");
      const positional = context.runCli(
        ["--pm-path", positionalRoot, "init", "app", "--json", "--yes"],
        { expectJson: true },
      );
      expect(positional.code).toBe(0);
      expect(positional.json.settings).toMatchObject({ id_prefix: "app-" });

      const equivalentRoot = path.join(context.tempRoot, "equivalent-prefix");
      const equivalent = context.runCli(
        [
          "--pm-path",
          equivalentRoot,
          "init",
      ...
      expect(equivalent.json.settings).toMatchObject({ id_prefix: "app-" });

      const conflict = context.runCli([
        "--pm-path",
        path.join(context.tempRoot, "conflicting-prefix"),
        "init",
        "app",
        "--id-prefix",
        "ops",
      ...
      expectJsonErrorEnvelope(conflict.stderr, {
        code: "init_id_prefix_conflict",
        exit_code: 2,
      });

      const empty = context.runCli([
        "--pm-path",
        path.join(context.tempRoot, "empty-prefix"),
        "init",
        "--id-prefix",
        " ",
        "--json",
      ...
      const driveRelative = context.runCli([
        "init",
        "--id-prefix",
        "C:tracker",
        "--json",
      ...
      expectJsonErrorEnvelope(driveRelative.stderr, {
        code: "init_id_prefix_path_like",
        exit_code: 2,
      });
    });
  ...
  it("treats path-like init positionals as target tracker paths without mutating the caller tracker", async () => {
    await withTempPmPath(async (context) => {
      ...
      const callerInit = context.runCli(
        ["--pm-path", callerPmPath, "init", "pm", "--json", "--yes"],
        {
          expectJson: true,
          cwd: projectRoot,
        },
      ...
      const callerSettingsBefore = await readFile(
        path.join(callerPmPath, "settings.json"),
        "utf8",
      );

      const callerExtensionsBefore = await readdir(
        path.join(callerPmPath, "extensions"),
      );
      const targetInit = context.runCli(
        [
          "init",
          targetPmPath,
          "--json",
      ...
      expect(targetInit.json.target).toEqual({
        mode: "tracker-path",
        tracker_root: targetPmPath,
      });
      ...
      for (const step of nextSteps) {
        const commands =
          step.match(/pm (?:--pm-path [^ ]+ )?[a-z][^,)]*/g) ?? [];
        expect(
          commands.every((command) =>
            command.startsWith(`pm --pm-path ${targetPmPath} `),
          ),
        ).toBe(true);
      }

      const scopedList = context.runCli(
        ["--pm-path", targetPmPath, "list", "--json"],
        { expectJson: true, cwd: targetPmPath },
      );
      const scopedContext = context.runCli(
        ["--pm-path", targetPmPath, "context", "--json"],
        { expectJson: true, cwd: targetPmPath },
      );
      expect(scopedList.code).toBe(0);
      expect(scopedContext.code).toBe(0);

      const callerSettingsAfter = await readFile(
        path.join(callerPmPath, "settings.json"),
        "utf8",
      );
      const callerExtensionsAfter = await readdir(
        path.join(callerPmPath, "extensions"),
      );
      const targetSettings = JSON.parse(
        await readFile(path.join(targetPmPath, "settings.json"), "utf8"),
      ) as {
        id_prefix?: string;
  ...
  it("initializes an explicit workspace target under .agents/pm", async () => {
    await withTempPmPath(async (context) => {
      ...
      const initialized = context.runCli(
        ["init", "acme", "--workspace", workspaceRoot, "--json", "--yes"],
        {
          expectJson: true,
        },
      );
      ...
      expect(
        JSON.parse(
          await readFile(path.join(trackerRoot, "settings.json"), "utf8"),
        ),
  ...
  it("guards explicit --pm-path values that point at a workspace root", async () => {
    await withTempPmPath(async (context) => {
      ...
      await writeFile(
        ...
        JSON.stringify({ name: "path-trap" }),
        "utf8",
      );

      const positionalGuarded = context.runCli([
        "init",
        workspaceRoot,
        "--json",
      ]);
      ...
      expect(envelope.recovery?.next_best_command).toContain(
        path.join(workspaceRoot, ".agents", "pm"),
      );
      ...
      const forced = context.runCli(
        ["--pm-path", workspaceRoot, "init", "--json", "--yes", "--force"],
        {
          expectJson: true,
        },
      );
  ...
  it("resolves root-layout trackers for bare invocations and extension pm_root context (GH-495)", async () => {
    await withTempPmPath(async (context) => {
      ...
      const init = context.runCli(["init", workspaceRoot, "--json", "--yes"], {
        expectJson: true,
      });
      expect(init.code).toBe(0);
      expect(init.json).toMatchObject({
        target: { mode: "tracker-path", tracker_root: workspaceRoot },
      });
      ...
      const bareList = runDirectDistCli(["list", "--json"], {
        cwd: workspaceRoot,
        env: bareEnv,
        expectJson: true,
      });
      ...
      const probe = runDirectDistCli(["probe", "root", "--json"], {
        cwd: workspaceRoot,
        env: bareEnv,
        expectJson: true,
      });
      ...
      const spawnBack = runDirectDistCli(
        ["--pm-path", workspaceRoot, "stats", "--json"],
        {
          cwd: workspaceRoot,
          env: bareEnv,
          expectJson: true,
