    ...
    await withTempPmPath(async (context) => {
      ...
      await writeTestExtension({
        ...
        entrySource: [
          ...
          `  api.hooks.beforeCommand((context) => appendFileSync(${JSON.stringify(hookLogPath)}, \`before:${"${context.command}"}\\n\`, "utf8"));`,
          `  api.hooks.afterCommand((context) => appendFileSync(${JSON.stringify(hookLogPath)}, \`after:${"${context.command}"}:${"${context.ok}"}\\n\`, "utf8"));`,
          "}",
          "export default { activate };",
          "",
        ].join("\n"),
