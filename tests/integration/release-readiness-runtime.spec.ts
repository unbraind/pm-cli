...
describe("release readiness runtime coverage", () => {
  ...
  it("keeps npm packaging allowlist and prepublish guard aligned", async () => {
    ...
    expect(packageJson.scripts?.["quality:static"]).toBe(
      "node scripts/release/static-quality-gate.mjs --min-docstring-coverage 92.93 --min-exported-docstring-coverage 84.41 --min-member-docstring-coverage 13.655 && node scripts/release/token-budget-gate.mjs",
    );
  ...
  it("keeps mutation-triggered search refresh wiring for test run-tracking paths", async () => {
    ...
    expect(opsSource).toContain(
      "values.addValues.length > 0 || values.addJsonValues.length > 0 || values.removeValues.length > 0 || options.run === true",
    );
