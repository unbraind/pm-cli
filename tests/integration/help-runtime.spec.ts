...
describe("CLI help runtime coverage (sandboxed)", () => {
  it("describes top-level help as a universal extensible CLI", async () => {
    await withTempPmPath(async (context) => {
      ...
      expect(detailedHelp.stdout).toContain("pm package install guide-shell --project");
  ...
  it("describes search help text with the auto default and explicit opt-in modes", async () => {
    await withTempPmPath(async (context) => {
      ...
      expect(help.stdout).toMatch(
        /Search mode: keyword\|semantic\|hybrid \(default:\s+auto\)/,
      );
