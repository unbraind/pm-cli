...
- [pm-52eh](.agents/pm/features/pm-52eh.toon)
- [pm-mcxr](.agents/pm/issues/pm-mcxr.toon)
- [pm-u42x](.agents/pm/issues/pm-u42x.toon)
...
- Mutations other than `claim` against items assigned to another assignee return conflict unless `--force`; optional packages may contribute narrower policy-specific bypasses without expanding the bare-core CLI or SDK contract.
...
| `pm release <ID>` | id, optional `--author`/`--message`/`--force`; optional packages may add narrow policy extensions | `{ item, released_by, previous_assignee, forced }` |
...
| `pm docs <ID> --add/--add-glob/--remove/--migrate/--validate-paths` | id + doc refs (`--add/--remove` accept CSV key/value, markdown `key: value`, or stdin token `-`); optional glob expansion via repeatable `--add-glob` (plain glob or `pattern=<glob>,scope=<scope>,note=<text>`); optional additive linked-path hygiene (`--migrate from=<old>,to=<new>`, path existence validation) | `{ id, docs, changed, count, migrations_applied, validation }` |
...
- Accept normal package-manager linked test scripts such as `pnpm test`, `npm run test`, `yarn test`, and `bun test` because linked execution injects sandbox `PM_PATH` and `PM_GLOBAL_PATH`; reject direct test-runner command entries (for example `vitest`, `node --test`, `npx vitest`, `pnpm dlx vitest`) unless they use `node scripts/run-tests.mjs ...` or explicitly set both sandbox env vars inline; chained direct test-runner segments are validated independently and rejected when not explicitly sandboxed.
...
- PM-driven test execution MUST use sandboxed `PM_PATH` and `PM_GLOBAL_PATH`; repo-level test gates should prefer the `node scripts/run-tests.mjs test|coverage` wrapper, while linked-test execution injects sandbox roots for stored package-manager scripts.
- `pm test <ID> --add` MUST enforce this by requiring `command` metadata (with optional `path` metadata), accepting package-manager scripts that are sandboxed at linked-test runtime, and rejecting direct test-runner command entries at add-time unless they use `node scripts/run-tests.mjs ...` or explicitly set both `PM_PATH` and `PM_GLOBAL_PATH`; chained direct test-runner segments that are not explicitly sandboxed remain rejected.
