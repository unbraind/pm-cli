...
- Mutations other than `claim` against items assigned to another assignee return conflict unless `--force` (comments can use additive `--allow-audit-comment` for append-only audit entries, and release can use `--allow-audit-release` for non-owner handoffs that only clear assignee metadata).
...
| `pm aggregate` | grouped-count governance query (`--group-by parent,type --count`) with list-like filters and optional `--include-unparented` | `{ groups, count, totals, filters, now, warnings? }` |
