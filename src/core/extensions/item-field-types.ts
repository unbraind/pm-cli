...
/** Suggest a known field type for a typo'd declaration (e.g. `"strnig"` -> `"string"`), using the shared OSA Damerau–Levenshtein helper so adjacent transpositions count as a single edit. Returns the first canonical type within the proportional edit budget (in declaration order — the kinds are distinct enough that at most one matches), or `null` when nothing is close enough. */
export function suggestKnownItemFieldType(
  value: string,
): KnownItemFieldType | null {
  const normalized = value.trim().toLowerCase();
