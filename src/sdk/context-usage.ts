...
import {
  appendEvents, readEvents, resolveNow, ContextUsageValidationError, CONTEXT_USAGE_LIMITS, type ContextUsageWriteReceipt,
} from "./context/usage-ledger.js";
export { ContextUsageValidationError, CONTEXT_USAGE_LIMITS, type ContextUsageWriteReceipt } from "./context/usage-ledger.js";
...
export interface ContextUsageServingReceipt {
  ...
  /** Persisted sample identities; absent on legacy unsampled receipts. */
  recorded_item_ids?: readonly string[];
  /** Measured bounded persistence; absent on legacy receipts. */
  storage?: ContextUsageWriteReceipt;
}
...
export type ContextUsageEvent =
  | {
      ...
      /** Full candidate population before deterministic prefix sampling. */
      candidate_count?: number;
      /** Candidate rows excluded from this bounded feedback sample. */
...
export interface ContextUsageLedgerOptions {
  ...
  maxEvents?: number;
  /** Physical byte ceiling; compaction retains at most half to provide headroom. */
  maxBytes?: number;
...
/** Records a bounded prefix of ranked context candidates with explicit sampling counts. */
export async function recordContextUsageServing(
  ...
  const recordedRows = normalizedRows.slice(0, CONTEXT_USAGE_LIMITS.max_serving_rows);
  const receipt: ContextUsageServingReceipt = {
    serve_id: randomUUID(),
    author: options.author.trim(),
    surface: options.surface,
    rows: normalizedRows,
    recorded_item_ids: recordedRows.map((row) => row.id),
  };
  receipt.storage = await appendEvents(options, [{
    kind: "serve",
    schema_version: 2,
    ...
    omitted_row_count: normalizedRows.length - recordedRows.length,
    result_omitted: false,
    packed_item_ids: recordedRows
...
export async function recordContextUsageDelivery(
  ...
  const recordedIds = new Set(options.receipt.recorded_item_ids ?? options.receipt.rows.map((row) => row.id));
  const delivery: ContextUsageEvent = {
    ...
    delivered_item_ids: deliveredItemIds.filter((id) => recordedIds.has(id)),
  };
