...
interface RuntimeActivationPaths {
  commandPathsByExtension: Map<string, Set<string>>;
...
function addRuntimePath(
  ...
  if (normalizedName.length === 0 || normalizedPath.length === 0) {
    return;
  }
  const existing = pathsByExtension.get(normalizedName) ?? new Set<string>();
  existing.add(normalizedPath);
  pathsByExtension.set(normalizedName, existing);
}

function collectRuntimeActivationPaths(
  activationResult: Awaited<ReturnType<typeof activateExtensions>>,
): RuntimeActivationPaths {
  ...
  for (const registration of activationResult.registrations.commands) {
    addRuntimePath(
      commandPathsByExtension,
      registration.name,
      registration.command,
    );
    addRuntimePath(
      actionPathsByExtension,
      registration.name,
      registration.action,
    );
  }
  for (const handler of activationResult.commands.handlers) {
    addRuntimePath(commandPathsByExtension, handler.name, handler.command);
  }
  for (const override of activationResult.commands.overrides) {
    addRuntimePath(commandPathsByExtension, override.name, override.command);
  }
  return { commandPathsByExtension, actionPathsByExtension };
}

function sortedRuntimePaths(
...
function resolveDoctorActivationStatus(params: {
  enabled: boolean;
  normalizedName: string;
  loadedNames: Set<string>;
  loadFailedNames: Set<string>;
  activationFailedNames: Set<string>;
}): Pick<ManagedExtensionSummary, "runtime_active" | "activation_status"> {
  const {
    enabled,
    normalizedName,
    loadedNames,
    loadFailedNames,
    activationFailedNames,
  } = params;
  if (!enabled) {
    return { runtime_active: false, activation_status: "not_loaded" };
  }
  if (
    loadFailedNames.has(normalizedName) ||
    activationFailedNames.has(normalizedName)
  ) {
    return { runtime_active: false, activation_status: "failed" };
  }
  if (loadedNames.has(normalizedName)) {
    return { runtime_active: true, activation_status: "ok" };
  }
  return { runtime_active: false, activation_status: "not_loaded" };
}
...
export function applyDoctorRuntimeActivationState(
  ...
  const loadFailedNames = new Set(
    loadResult.failed.map((entry) =>
      normalizeExtensionNameForMatch(entry.name),
    ),
  );
  const activationFailedNames = new Set(
    activationResult.failed.map((entry) =>
      normalizeExtensionNameForMatch(entry.name),
    ),
  );
  const { commandPathsByExtension, actionPathsByExtension } =
    collectRuntimeActivationPaths(activationResult);

  return extensions.map((entry) => {
    const normalizedName = normalizeExtensionNameForMatch(entry.name);
    const commandPaths = sortedRuntimePaths(
      commandPathsByExtension.get(normalizedName),
    );
    const actionPaths = sortedRuntimePaths(
      actionPathsByExtension.get(normalizedName),
    );
    const runtimeMetadata = {
      ...(commandPaths ? { command_paths: commandPaths } : {}),
      ...(actionPaths ? { action_paths: actionPaths } : {}),
    };
    const activationStatus = resolveDoctorActivationStatus({
...
function lifecycleFlagCommand(
  options: ExtensionCommandOptions,
  action: string,
): string {
  return options.vocabulary === "package"
    ? `pm package --${action}`
    : `pm extension --${action}`;
}
...
function parseRegistrationCollisionWarning(
  warning: string,
): RegistrationCollisionWarningParts | null {
...
function collectRegistrationCollisionExtensionNames(
  warnings: string[],
): string[] {
...
export function buildRegistrationCollisionRemediation(
  ...
  const registrationCollisionWarnings = warnings.filter(
    isRegistrationCollisionWarning,
  );
  if (registrationCollisionWarnings.length === 0) {
    return null;
  }
  const collisionNames = collectRegistrationCollisionExtensionNames(
    registrationCollisionWarnings,
  );
  const collisionNameText =
    collisionNames.length > 0
      ? ` Conflicting extensions: ${collisionNames.join(", ")}.`
      : "";
  return (
...
function buildRegistrationCollisionPlan(
  ...
  const collisions = warnings
    ...
    .filter(
      (entry): entry is RegistrationCollisionWarningParts => entry !== null,
    )
    .sort((left, right) =>
  ...
  for (const collision of collisions) {
    for (const name of [collision.winner.name, collision.displaced.name]) {
      ...
      affectedByName.set(
        normalizedName,
        (affectedByName.get(normalizedName) ?? 0) + 1,
      );
    }
  }
  const remediationCandidates = [...affectedByName.entries()]
    .map(([normalizedName, affectedCollisions]) => {
      ...
      return {
        ...
        feature_loss: {
          command_paths: [...commandPaths].sort((left, right) =>
            left.localeCompare(right),
          ),
          action_paths: [...actionPaths].sort((left, right) =>
            left.localeCompare(right),
          ),
        },
      };
    ...
    .sort((left, right) => {
      const affectedDelta =
        right.affected_collisions - left.affected_collisions;
      if (affectedDelta !== 0) {
        return affectedDelta;
      }
      const leftSurfaceCount =
        left.feature_loss.command_paths.length +
        left.feature_loss.action_paths.length;
      const rightSurfaceCount =
        right.feature_loss.command_paths.length +
        right.feature_loss.action_paths.length;
...
export function classifyDoctorLoadFailureWarnings(
  loadFailures: Array<{ name: string; error: string }>,
): string[] {
  ...
  for (const failure of loadFailures) {
    ...
    if (
      ...
      warnings.push(
        `extension_load_failed_sdk_dependency_missing:${failure.name}`,
      );
    }
    if (
      ...
      warnings.push(
        `extension_load_failed_module_mode_mismatch:${failure.name}`,
      );
    }
  }
...
export function classifyDoctorActivationFailureWarnings(
  activationFailures: Array<{
    name: string;
    trace?: { missing_capability?: string; capability?: string };
  }> = [],
): string[] {
  ...
  for (const failure of failures) {
    ...
    const missingCapability =
      failure.trace?.missing_capability ?? failure.trace?.capability;
    if (
      typeof missingCapability === "string" &&
      missingCapability.trim().length > 0
    ) {
      warnings.push(
        `extension_capability_missing:${failure.name}:${missingCapability.trim().toLowerCase()}`,
      );
    }
  }
  return [...new Set(warnings)].sort((left, right) =>
...
export function classifyUnusedCapabilityWarnings(
  ...
  const warnings: string[] = [];
  for (const entry of reconcileExtensionCapabilityUsage(
    loadResult.loaded,
    activationResult,
  )) {
    for (const capability of entry.unused) {
      warnings.push(
        `extension_capability_unused:${entry.layer}:${entry.name}:${capability}`,
      );
    }
  }
  return [...new Set(warnings)].sort((left, right) =>
...
/** Inputs every triage remediation message can reference: the lifecycle command options and resolved `scopeFlag`/`scope`, plus the two numeric totals (`updateCheckFailedTotal`, `policyWarningCount`) whose own warning-driven rules fire purely on a count rather than a warning prefix. */
...
interface TriageRemediationRule {
  matches: (
    ...
    ctx: TriageRemediationContext,
  ) => boolean;
  build: (ctx: TriageRemediationContext) => string;
}

/** Ordered remediation rules evaluated while at least one warning is present. The order is load-bearing — it is the order remediation hints appear in doctor output — so new rules must be inserted at the intended position rather than appended. */
const TRIAGE_REMEDIATION_RULES: readonly TriageRemediationRule[] = [
  {
    matches: (warnings) =>
      warnings.some((warning) => warning.startsWith("extension_manifest_")),
    build: (ctx) =>
  ...
  },
  {
    matches: (warnings) =>
      warnings.some((warning) =>
        warning.startsWith("extension_capability_missing:"),
      ),
    build: (ctx) =>
  ...
  },
  {
    matches: (warnings) =>
      warnings.some((warning) =>
        warning.startsWith("extension_capability_unused:"),
      ),
    build: (ctx) =>
  ...
  },
  {
    matches: (warnings) =>
      warnings.some((warning) =>
        warning.startsWith("extension_schema_narrow_activation:"),
      ),
    build: () =>
  ...
  },
  {
    matches: (warnings) =>
      warnings.some((warning) =>
        warning.startsWith("extension_load_failed_sdk_dependency_missing:"),
      ),
    build: (ctx) =>
  ...
  },
  {
    matches: (warnings) =>
      warnings.some((warning) =>
        warning.startsWith("extension_load_failed_module_mode_mismatch:"),
      ),
    build: (ctx) =>
  ...
  },
  {
    matches: (warnings) =>
      warnings.some(
        (warning) =>
          warning.startsWith("extension_pm_min_version_") ||
          warning.startsWith("extension_pm_max_version_"),
      ),
    build: (ctx) =>
  ...
  },
  {
    matches: (warnings) =>
      warnings.some((warning) =>
        warning.startsWith("extension_manager_state_"),
      ),
    build: (ctx) =>
      `Review and repair ${ctx.scope} managed extension state file if schema/read warnings persist.`,
  },
  {
...
function buildExtensionTriageRemediation(params: {
  ...
  const ctx: TriageRemediationContext = {
    options,
    scope,
    scopeFlag,
    updateCheckFailedTotal,
  ...
  if (updateAvailableTotal > 0) {
    remediation.push(
      `Update available managed extensions via ${lifecycleFlagCommand(options, "install")} ${scopeFlag} <source>.`,
    );
  }
  if (remediation.length === 0) {
    remediation.push(
      `No immediate action required. Re-run ${lifecycleFlagCommand(options, "manage")} ${scopeFlag} after extension changes.`,
    );
  }
  return remediation;
}
...
export function buildExtensionTriageSummary(
  ...
  const warningCodes = [
    ...new Set(effectiveWarnings.map((value) => warningCode(value))),
  ].sort((left, right) => left.localeCompare(right));
  const policyWarnings = summarizePolicyWarnings(effectiveWarnings);
  const scopeFlag = scope === "global" ? "--global" : "--project";
  const collisionPlan = buildRegistrationCollisionPlan(
    scope,
    normalizedWarnings,
    extensions,
    options,
...
export function parseDoctorDetailMode(
  ...
  throw new PmCliError(
    `Invalid --detail value "${raw}". Expected summary or deep.`,
    EXIT_CODE.USAGE,
  );
}
...
export function collectUnknownCapabilityGuidance(
  warnings: string[],
...
export function buildDoctorConsistencySummary(
  ...
  const failedLoadNames = new Set(
    failedLoads.map((entry) => normalizeExtensionNameForMatch(entry.name)),
  );
  const missingActiveProjectNames = activeProjectNames
    .filter(
      (name) =>
        !loadedProjectNames.includes(name) && !failedLoadNames.has(name),
    )
    .sort((left, right) => left.localeCompare(right));

  const warnings =
    missingActiveProjectNames.length > 0
      ? [
