...
/** Drop all memoized workspace extension registrations so the next `getWorkspaceContracts` call re-loads and re-activates extensions from disk. Long-lived hosts should call this after any extension or settings mutation. */
export function clearWorkspaceContractsCache(): void {
  workspaceExtensionRegistrationsCache.clear();
}

/** Memoize one workspace extension-registration load and evict rejected entries so transient activation failures do not poison long-lived embedding hosts. */
export function memoizeWorkspaceExtensionRegistrations(
  ...
  if (
    workspaceExtensionRegistrationsCache.size >= WORKSPACE_CONTRACTS_CACHE_LIMIT
  ) {
