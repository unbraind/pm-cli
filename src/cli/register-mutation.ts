...
type SchemaCommandResult = Awaited<ReturnType<SchemaCommandModule["runSchemaList"]>>
  ...
  switch (normalizedSubcommand) {
    ...
    case "add-status":
      return schema.runSchemaAddStatus(
        typeName,
        { role: inputs.roles, alias: inputs.aliases, description, order: inputs.order, author, force },
        globalOptions,
      );
    case "remove-status":
    ...
    default:
      ...
      if (options.infer === true) {
        return schema.runSchemaInferTypes(
          { minCount: inputs.minCount, apply: Boolean(options.apply), author, force },
          globalOptions,
        );
      }
 ...
 */
function renderSchemaResultHuman(schema: SchemaCommandModule, result: SchemaCommandResult): void {
...
export function registerMutationCommands(program: Command): void {
  ...
  filesCommand
    ...
    .option("--remove <value>", "Remove linked file by path only (path=<value>, path:<value>, plain path, or - for stdin); does not accept note=/scope= — record removal context with --message", collect)
  ...
  program
    ...
    .option("--remove <value>", "Remove linked doc by path only (path=<value>, path:<value>, plain path, or - for stdin); does not accept note=/scope= — record removal context with --message", collect)
