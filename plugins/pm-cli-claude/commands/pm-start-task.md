---
description: Start a pm-tracked task — orient, find or create the right item, claim it, and link initial scope. Accepts an optional item ID or keywords as argument.
---
...
5. **Add a start comment** with `pm_comments`: `"Starting work. Scope: <brief description of planned approach>."`
6. **Report** the claimed item's title, ID, and acceptance criteria.

If no matching item exists and the user wants to create one, collect title + description + type then call `pm_create`.
