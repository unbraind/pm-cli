...
async function loadRuntimeModule() {
  ...
  for (const packageRoot of resolvePackageRootCandidates()) {
    ...
    for (const modulePath of modulePaths) {
      ...
      try {
        cachedRuntimeModule = await import(pathToFileURL(modulePath).href);
        return cachedRuntimeModule;
      } catch {
  ...
  try {
    cachedRuntimeModule = await import(pathToFileURL(localRuntimePath).href);
    return cachedRuntimeModule;
  } catch {
...
async function runCalendarFromRuntime(options, global) {
...
function renderCalendarOutput(context) {
...
function calendarCommand(name) {
  return {
    ...
    flags: [...calendarFlags],
    run: async (context) => runCalendarFromRuntime(context.options, context.global),
  };
}
