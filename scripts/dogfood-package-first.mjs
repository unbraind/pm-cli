...
try {
  ...
  assert(
    packageScaffold?.details?.extension?.command === "starter scaffold package ping",
    "package init scaffold did not report reserved-root starter command",
  );
  ...
  const scaffoldInvoke = run("package scaffold command", ["starter", "scaffold", "package", "ping"]);
  assert(scaffoldInvoke?.ok === true, "scaffolded package command did not execute");
  assert(scaffoldInvoke?.command === "starter scaffold package ping", "scaffolded package command reported unexpected command");
