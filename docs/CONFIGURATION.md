...
| `governance.duplicate_detection_mode`      | create/copy similarity policy: `off`, `advisory`, or `strict`; the minimal preset uses `off`, while the default and strict presets use `strict`                                                       |
...
threshold match. The strict check and authoritative item/index persistence share
one cross-process mutation lock, so concurrent equivalent creates cannot both
commit. Kebab-case config keys are the documented spelling; existing
...
| `PM_TELEMETRY_READ_SAMPLE_RATE` | number greater than `0` and at most `1` | Override automatic successful-read sampling; `1`, empty, or invalid values retain every invocation. Omit for automatic frequency bounding. |
...
Repeated core reads are sampled automatically after the first ten invocations of a command in each UTC minute. The nth invocation has successful-pair inclusion probability `min(1, (10 / n)^2)`, keeping expected successful pairs below twenty per command per minute. This is a statistical bound, not a hard cap; failures remain retained. Counters are shared across processes under the installation queue mutex, reset at the next minute, and store only the eight fixed command names and numbers in `runtime/telemetry/read-sampling.json`. Unavailable state retains the invocation.

Eligible commands are `stats`, `ops stats`, `list`, `get`, `next`, `context`, `activity`, and `aggregate`. The runtime must establish core ownership: `--no-extensions` provides an explicit boundary, while an active core command may coexist with unrelated extension commands. Matching handlers or parsers, and active hooks, services, preflights, renderers, migrations, search/vector adapters, or assurance providers prevent sampling. Unknown ownership and mutations retain every invocation. Set `PM_TELEMETRY_READ_SAMPLE_RATE=1` to disable automatic sampling, or a number greater than zero and below one for a fixed rate; explicitly empty or invalid values also retain every invocation.

Successful sampled reads defer their start until completion so both lifecycle events are retained or dropped together; abruptly terminated sampled reads have no event pair. Retained pairs carry numeric `sample_rate`, and OTLP spans carry `pm.sample_rate`; estimate population totals with weight `1 / sample_rate` (missing means `1`). Very small inclusion probabilities produce high-variance estimates. Failed sampled reads carry probability `1`. Local queue statistics describe retained observations, not weighted population estimates.
...
Status definitions are authored in `.agents/pm/schema/statuses.json`; the
`schema.statuses` projection in `settings.json` is a deterministic runtime
summary, not a second registry to edit independently. Status ids and aliases are
normalized to underscore tokens in both projections (for example,
`in-progress` becomes `in_progress`) while CLI input continues to accept
underscore, hyphen, and space spellings through the resolved status registry.
