...
`RelationshipEventStore` is the built-in durable filesystem adapter. It stores validated JSONL at `.agents/pm/relationships/events.jsonl` by default, replays every row through the same registry and cardinality checks on open, and serializes cross-process appends with the tracker lock. Async `snapshot()` and `page()` reads refresh from durable history so long-lived readers observe other processes' completed appends. Callers can select another contained relative path for domain-specific ledgers. Database, replicated-log, and event-bus adapters can persist the same public events and rebuild the same snapshots.
