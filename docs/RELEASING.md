...
- release at most once per UTC day after a successful publish; if the scheduled run failed before publishing, a non-`github-actions[bot]` closure of the exact bot-created `Auto Release blocked` issue on the same UTC day triggers one production retry attempt so a changed tree still gets a daily release
...
- closing the exact bot-created `Auto Release blocked: scheduled run failed` issue as a maintainer or agent retries Auto Release once for that same-day closure on the production path (`push=true`, `dry_run=false`), records a same-day retry marker before running the release pipeline, comments with the published tag when the retry publishes, comments with a skip reason when the retry succeeds without publishing a new tag or is safely refused before release mutation, and reopens the same issue if the retry actually fails; a second close of the reopened issue on the same UTC day is refused before release mutation and reported as `retry_already_attempted` so the production retry cannot loop, and workflow cleanup closures by `github-actions[bot]` are ignored so a successful scheduled cleanup cannot retrigger release
