...
- `CHANGELOG.md` is generated by the latest npm `pm-changelog` package (`pm install npm:pm-changelog --project`, then `pm changelog generate`) from closed tracker items and checked in CI; do not edit it by hand
...
2. version bump
3. latest `pm-changelog` install and main changelog refresh through `pm changelog generate`
