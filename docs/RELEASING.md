...
- `CHANGELOG.md` is generated by the latest npm `pm-changelog` package (`pm install npm:pm-changelog --project`, then `pm changelog generate --mode prepend`) from closed tracker items in the package-owned release tag range and checked in CI; do not edit it by hand
...
2. version bump
3. latest `pm-changelog` install and main changelog refresh through `pm changelog generate --mode prepend --release-version-from-package --since-previous-tag --until-release-tag`
