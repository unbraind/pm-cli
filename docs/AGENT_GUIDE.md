...
- `pm init` maintains a fenced `.gitignore` block for `.agents/pm/runtime/` and `.agents/pm/search/`. If those caches were committed before initialization, remove them from the index once with `git rm -r --cached .agents/pm/runtime .agents/pm/search`.
