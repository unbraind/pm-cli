...
- Streamable HTTP requires `MCP-Protocol-Version`, `Mcp-Method`, and
  `Mcp-Name`; schema-declared `x-mcp-header` values are encoded, decoded, and
  compared with tool arguments before dispatch. Header/version mismatch uses
  code `-32020` and HTTP 400.
