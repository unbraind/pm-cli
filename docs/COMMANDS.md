...
`pm update` can make non-lifecycle metadata updates and append comments, linked files, and linked
...
`pm gc --scope` accepts `index`, `embeddings`, `runtime`, `locks`, and `checkpoints`, comma-separated or repeated. Omitting the flag selects all scopes:

- `runtime` clears `runtime/test-runs/` and `runtime/history-drift-cache.json`. The next health check rescans history drift.
...
`--auto-fix` reports `planned_fixes`, `applied_fixes`, `gated_fixes`, and `failed_fixes` under `fixes`. Each row identifies the item, check, field, and equivalent standalone command:

- Safe field backfills derive a missing terminal `resolution` from `close_reason` (falling back to `"completed"`) or a missing `close_reason` from existing `resolution`. Every applied change uses the audited update path; auto-fix never closes, cancels, or deletes items.
- `--fix-scope lifecycle` explicitly permits reparenting an active item from its terminal parent to an active grandparent, or clearing that parent link. These structural fixes are otherwise planned only.
- `--fix-scope estimates` explicitly permits heuristic per-type estimates. Defaults in minutes are Epic/Milestone 2880, Feature/Story 480, Task/Plan 120, Issue/Bug 60, Chore 30, Decision 15, and fallback 120. Override `validation.estimate_defaults_by_type` with a type-to-minutes map.
- `--fix-scope` is an exact, repeatable or comma-separated allowlist: `metadata`, `resolution`, `estimates`, `lifecycle`. Omitting it grants only the safe metadata/resolution scopes; specifying only estimates grants only estimates.
- `--dry-run` previews without mutation. With no explicit check flags, auto-fix runs only metadata, resolution, and lifecycle checks. Output checks describe the pre-fix state; rerun validation to confirm convergence.
...
`pm validate --check-files` joins stale paths, owning items, and relocation candidates in structured `details.missing_linked_path_rows` in both compact and full output:

- `moved` means same-basename candidates exist; these are suggestions requiring verification. `deleted` means none exists. `malformed` marks ambiguous missing CSV or path:note literals and never infers a relocation or prune.
- Counters distinguish unique paths from linked holder/field pairs, active holders from the closed subset, and all terminal holders. Only active missing paths cause the files warning; historical debt remains visible. Custom lifecycle roles determine the partition.
- Active paths and owners appear first. Compact output bounds both dimensions and includes total counts and truncation flags; `--verbose-file-lists` restores every row. See [Linked-file repair rows](OUTPUT_PROJECTION_CONTRACTS.md#linked-file-repair-rows) for the SDK contract.
- Existing unlinked paths retain `orphaned_path_classifications` and `orphaned_path_rows`, with `docs_unowned`, `tests_unowned`, `source_unowned`, or `unlinked_existing` classifications and concrete ownership hints where available.
- `--prune-missing` removes only links classified `deleted`, retaining moved and malformed references for review. It never removes real files, honors `--dry-run`, implies `--check-files`, and records equivalent files/docs removal commands in `fixes.applied_fixes`.
