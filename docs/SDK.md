...
- Multi-branch merge primitives: `mergeItemDocuments`, `mergeHistoryStreams`, `mergeJsonDocuments`, `runMergeDriver`, and `runMergeInstall` provide the same field-aware item, hash-chain-preserving history, and key-level configuration semantics as `pm merge`. See [Multi-Branch Merge Safety](MERGE_SAFETY.md).
