...
- A block-enforced ratchet ceiling stops resolving when its population reaches zero, so the gate fails with an unhandled error on the day the remediation it enforces succeeds ([pm-xmmafu](https://github.com/unbraind/pm-cli/blob/main/.agents/pm/issues/pm-xmmafu.toon))
